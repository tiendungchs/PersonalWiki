...
Difficulty reads off the row weight (more ✓ = harder), but this is a **derived difficulty ordering, not a partition**: FrontierMath sets three bits at once (edge-labels + path + vocabulary). Full axes and reading notes in [[wiki/concepts/latent-graph-discovery.md]]. The distinction a reader most often conflates: **edge-label latent** = *single-hop* rule identity from many (start, end) pairs with vocabulary being learned (ARC-AGI); **path latent** = *multi-hop* composition over an *already-known* move-set (navigation, proof search).
