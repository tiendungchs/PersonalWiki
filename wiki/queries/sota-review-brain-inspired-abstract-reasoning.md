...
updated: 2026-07-18
sources: []
related: [wiki/queries/central-framing-epistemic-audit.md, wiki/queries/mec-abstract-codes-vs-declarative-rules.md, wiki/overview.md, wiki/concepts/latent-graph-discovery.md, wiki/concepts/structural-generalization.md, wiki/concepts/abstract-reasoning.md, wiki/concepts/factorized-representations.md, wiki/concepts/two-learning-timescales.md, wiki/concepts/planning-as-inference.md, wiki/concepts/neural-manifolds.md, wiki/concepts/shortcut-reasoning.md, wiki/concepts/intelligence-density.md, wiki/concepts/memory-schemas.md, wiki/concepts/world-models.md, wiki/concepts/refinement-loops.md, wiki/entities/grid-cells.md, wiki/entities/hippocampal-entorhinal-system.md, wiki/entities/prefrontal-cortex.md, wiki/entities/basal-ganglia.md, wiki/entities/default-mode-network.md, wiki/entities/insect-central-complex.md, wiki/entities/tem-model.md, wiki/entities/arc-agi.md, wiki/entities/spacetime-attractor.md, wiki/entities/vsa-model.md, wiki/entities/jepa-model.md, wiki/entities/vl-jepa-model.md, wiki/entities/dinov2-model.md, wiki/entities/dinov3-model.md, wiki/entities/equilibrium-propagation.md, wiki/entities/baba-is-ai.md, wiki/entities/frontiermath-benchmark.md, wiki/entities/pgm-benchmark.md, wiki/entities/gpqa-benchmark.md, wiki/entities/olymmath.md, wiki/queries/building-blocks-mec-hc-pfc.md, wiki/queries/brain-inspired-vs-solver-approach.md, wiki/papers/math-reasoning-survey-2026.md, wiki/queries/reasoning-as-coupled-navigation-strategizing.md, wiki/concepts/relational-reinterpretation.md, wiki/concepts/recursion.md, wiki/concepts/core-knowledge.md, wiki/entities/dnc-model.md, wiki/entities/vector-hash-model.md, wiki/entities/mlc-model.md, wiki/entities/cscg-model.md, wiki/papers/das-compositional-neurosymbolic-arc-2025.md, wiki/papers/lieberman-meyer-mpfc-subdivisions-2019.md, wiki/papers/heidbreder-groenewegen-mpfc-dorsoventral-2003.md, wiki/queries/proposed-reasoning-model-block-architectures.md, wiki/queries/sota-review-brain-inspired-abstract-reasoning-fr.md, wiki/queries/action-semantics-contextual-inference.md, wiki/concepts/contextual-inference.md]
---

# State-of-the-Art Review: Brain-Inspired Architectures for Abstract Reasoning via Latent Graph Discovery

> **Purpose of this document.** A self-contained review of the field as it stands, written to lay out the foundation for a specific methodological direction and to justify the design choices that follow from it. It synthesises the wiki's concept, entity, and paper pages into a single narrative: what the target capability is, why current systems fail to reach it, what neuroscience contributes as a blueprint, and — given all of this — which architectural path is best supported by the evidence. Citations are given as *(Author et al. year)*.

---

## Abstract

Deep learning has saturated most i.i.d. benchmarks yet still fails at out-of-distribution *systematic generalization* — the ability to recombine known structure in novel ways. Human and animal brains do not share this failure. This review argues that (i) the diverse tasks grouped under "abstract reasoning" reduce to a single computational problem, **latent graph discovery** — inferring the hidden relational structure of a domain from observations and then navigating it; (ii) the reason current architectures fail is not scale or data but a missing *inductive structure* — specifically the factorization of relational structure from sensory content, which places the required generalization outside a monolithic model's reachable manifold *by construction*; and (iii) the mammalian entorhinal–hippocampal–prefrontal (MEC/HC/PFC) system provides a validated, convergent blueprint for the missing structure. We review the benchmark evidence isolating the bottleneck (ARC-AGI, FrontierMath, PGM, ConceptARC, Baba Is AI), the brain-inspired principles that address it, and the biologically-plausible learning algorithms that could train such a system. We conclude that the evidence *justifies* the brain-inspired direction and specifies its shape rather than a single missing part: a **factorized, two-timescale world model in the lineage of the Tolman–Eichenbaum Machine (TEM; Whittington et al. 2020)**, whose defining characteristics are read directly off the brain's reasoning circuitry — structure/content factorization, two-timescale W/M learning, latent-state path integration, hierarchical rule-of-rules control, planning-as-inference, and neuromodulatory goal bootstrapping, run as a coupled Navigator⇄Strategist loop. TEM instantiates only the *known-graph* half; a full reasoning model must additionally infer latent transformation rules, co-discover its own operator vocabulary, bootstrap goals with no external verifier, and ground an objectness prior — a *set* of interacting gaps, not a single one. We further show that the two leading *non*-brain-inspired programs — self-supervised world models (JEPA/DINO, §4.5) and solver-plus-external-verifier stacks (§4.6) — are complementary rather than competing: each empirically corroborates part of the thesis while leaving the core gaps (latent transformation inference, vocabulary co-discovery, verifier-free goal inference) open, and the brain-inspired generator can plug into the same external verifier the solver stack already uses. The rationale for each choice is stated explicitly.
...
Nodes = states; edges = transformations/actions; edge labels = the rule applied (often unknown); topology = the relational skeleton. The graph is recovered from sequences of *(observation, action, next-observation)* triples.

**Taxonomy — what is latent** (this is more principled than enumerating domains, because it predicts which tasks share computational structure). The taxonomy is a set of *independent* latent variables (node content, edge existence, edge labels, edge **vocabulary**, path, goal node); a task is a *subset* of hidden bits — a **bit-vector**, not one exclusive type — so benchmarks legitimately span multiple columns. Crossing the canonical benchmark families against the six variables (✓ latent · ◐ partial · — given):

| Benchmark family (examples) | Node content | Topology | Edge labels | Vocabulary | Path | Goal node |
|---|---|---|---|---|---|---|
| **Rule induction** — ARC-AGI, IQ/analogy | — | — | ✓ | — | — | — |
| **Interactive rule + goal** — ARC-AGI-3 | ◐ | — | ✓ | ✓ | ◐ | ✓ |
| **Path, known vocab** — Navigation; AIME/HMMT; MATH | — | ◐ | — | — | ✓ | — |
| **Path, partial vocab** — OlymMATH-HARD | — | — | — | ◐ | ✓ | — |
| **Node-content-latent QA** — GPQA; algebra, physics | ✓ | — | — | — | ◐ | — |
| **Edges + path + vocab** — FrontierMath | ◐ | — | ✓ | ✓ | ✓ | — |
| **Topology discovery** — scientific/causal discovery | ◐ | ✓ | ✓ | ✓ | ✓ | ◐ |

Difficulty reads off the row weight (more ✓ = harder), but this is a **derived difficulty ordering, not a partition**: FrontierMath sets three bits at once (edge-labels + path + vocabulary). One important distinction: **edge-label latent** = *single-hop* rule identity from many (start, end) pairs with vocabulary being learned (ARC-AGI); **path latent** = *multi-hop* composition over an *already-known* move-set (navigation, proof search).
...
| ------------------------------------- | -------------------------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------- |
| **ARC-AGI-1**                         | fell to o3 (~88%)                      | 2 (unknown vocabulary), 1 (two-level entanglement) | Fast *within-episode* adaptation > pretraining scale (resisted ~50,000× scale-up)      |
...
|---|---|---|---|
| **DINO** | *Perception front-end* | DINOv2 (1.1B), DINOv3 (7B + distilled) | Self-supervised ViT foundation models (EMA teacher-student + masked-patch iBOT + KoLeo/Gram). **Not world models** — frozen patch features that transfer to segmentation/depth/tracking. |
| **JEPA** | *World model* | I-JEPA → V-JEPA 2 (1B, 1M h video) → V-JEPA 2-AC (action-conditioned) → VL-JEPA → LeJEPA/LeWorldModel | Predict *representations* of masked/future states, not pixels; V-JEPA 2-AC adds a Mode-2 MPC planning loop. |

**Where the pivot corroborates this review** (independent empirical support for the framing, from a non-brain-inspired camp):

- **Representation-space prediction beats generation.** VL-JEPA-SFT (1.6B) beats GPT-4o/Claude-3.5/Gemini-2.0 on WorldPrediction-WM *without generating a token* (Chen et al. 2025); V-JEPA 2-AC beats pixel-space Cosmos 65–80% vs. 0% on real-robot manipulation (Assran et al. 2026). This is exactly the §5 claim (predict `s_y`, not `y`).
- **SSL from observation, not reward** — LeCun's central thesis, matching the observation-only regime of latent graph discovery (§2).
- **DINO supplies the missing objectness prior.** DINOv2 patch-token PCA yields emergent object-part decomposition with *no labels* (Oquab et al. 2023) — a candidate solution to the Core-Knowledge/objectness gap that ConceptARC diagnoses (§3) and that §8.3 flags as a risk for the sensory front-end (Blocks 1A+2A).
...
| **Latent transformation inference** (Block 3A) | ❌ forward-only (`s_x→ŝ_y`); `z` is a *minimum-information* latent (VICReg-regularized), not a discovered rule vocabulary | Cannot do Type-2 / ARC-AGI edges |
...
| **4 — Simultaneity** | Test-time search | Refinement loops: TTT, MCTS, expert iteration, evolutionary program search |
...
- **The verifier-availability boundary is empirically load-bearing.** The discovery track works *precisely* where an external checker exists (Python fitness, Lean kernel) and degenerates to unguided search where it does not — exactly the refinement-loop prediction, now confirmed at research scale.
...
| Structural robustness | ❌ generator pattern-matches trajectories | GSM-Symbolic NoOp −65% (Mirzadeh et al. 2024), MATH-Perturb hard −12–28% (Huang et al. 2025); a verifier can *reject* but not *supply* structure |
| Sample / energy efficiency | ❌ brute-force search (o3-high ≈ $30k/ARC task; ARC Prize 2025) | The Frostbite / intelligence-density gap (§1) restated on the compute axis |
...
|---|---|---|---|
| **MEC grid code** | Hexagonal grid-like fMRI signal during abstract 2D *conceptual* navigation, non-spatial (Constantinescu et al. 2016) | Structural code `g`; path-integration basis | 1A/1B |
| **Hippocampus** | Transitive/relational inference, schema-based one-shot generalization; TEM zero-shot structural transfer (Whittington et al. 2020) | Bind `p = f(g,x)`; fast instance-graph memory M | 2A–2D |
| **vmPFC / mPFC schemas** | *Strongest* abstract grid signal sits in vmPFC (Constantinescu 2016); schemas support transitive inference + assimilation/accommodation (de Sousa et al. 2026); amodal value & mental-state codes cross-classify across stimulus class in AMPFC/DMPFC (Lieberman et al. 2019) | Slow-W meta-graph; schema selection/update | 3B/3C context |
| **Lateral PFC** (rostro-caudal BA-8→9/46→10) | Parallel hierarchical rule search with step-wise (compression) learning curves (Badre et al. 2010); abstract remappable category tuning in 5–15 trials (Miller et al. 2002); model-based behavior emergent from model-free training (Wang et al. 2018) | Hierarchical rule-of-rules control; edge-label / vocabulary induction | 3A/3C |
| **Basal ganglia** | Go/NoGo gating of WM updates; neuromodulators as RL metaparameters (Doya 2002); dual-loop hypothesis-testing vs. procedural (COVIS) | Gating, credit assignment, exploration temperature | 3B/3D |
| **DMN** | Hosts abstract grid-code signals; integrates episodic + semantic + self into "frames of thought"; uniquely broadcasts to all cortical types (Paquola et al. 2025) | Contextual buffer integrating meta-graph context | 3B |
| **Insect central complex** | Ring-attractor heading + path integration confirmed in vivo (Seelig & Jayaraman 2015); convergent validation of the structural-update motif | Path-integration substrate (structural-code update) | 1B |
...
An RNN updating `g` under actions de-aliases observations and compresses rule-learning from O(transitions) to O(relation types). Confirmed in vivo as a ring attractor in the *Drosophila* central complex (Seelig & Jayaraman 2015).

### 5.4 Gate and modulate via basal ganglia / neuromodulation

Credit assignment in the brain is *structurally opponent*: a single phasic dopamine event drives D1→LTP (Go) and D2→LTD (NoGo) simultaneously (Gerfen & Surmeier 2011). Doya (2002) maps DA=TD-error, 5-HT=discount/horizon, NA=inverse-temperature/exploration, ACh=learning-rate — a closed meta-learning loop. Wang et al. (2018) show dopamine-trained slow weights give rise to a within-episode RL algorithm in PFC recurrent activity: **model-based behavior emergent from model-free training** — the biological instantiation of learning-to-learn; the underlying goal-directed/habit (≈ model-based/model-free) split is anatomically dissociated in rodent prelimbic vs. infralimbic mPFC (Heidbreder & Groenewegen 2003).
...
- **Contextual inference over an action→outcome repertoire** (Heald, Lengyel & Wolpert 2021, COIN; Haruno et al. 2001, MOSAIC): the brain does not learn *what an action does*; it maintains a repertoire of context-conditioned action→outcome models and infers a posterior over which one applies now, with a nonparametric (Dirichlet-process) prior that grows the repertoire when no context explains the data. This is the neuroscience mechanism for adapting to **changing action semantics** (source 2): the action effect `v_a` is a *latent variable inferred at play-time*, not a trained parameter — and most apparent within-episode "learning" is re-weighting a fixed repertoire, no plasticity. It is the brain-side substrate for Block 3A's Transformation Inferrer and localizes the strategy layer above rules/mappings (Murray, Bussey & Wise 2000).
...
| Learned reasoning algorithm (System II) | ✅ native (reasoning) · ◐ grafted (planning) | Model-based behavior emerges natively from model-free training in PFC recurrence (Wang et al. 2018); the *parallel planner* is not in base TEM — it is grafted via the STA (§5.7), which is why the §8.0 matrix scores TEM's planning-as-inference row ❌ˢ ("supplied via STA"), not ✅ |
...
**The asymmetry.** Every requirement JEPA and the solver miss on their crux rows, the brain blueprint supplies *natively* — and the rows where the brain core is *also* incomplete (objectness front-end; latent transformation inference) are qualitatively different from the other camps' misses: they are **grafts onto an architecture that already satisfies the surrounding requirements**, not re-foundings of the paradigm. That — together with the further gaps a full model must still close (goal bootstrapping without a verifier, vocabulary transfer, multi-level meta-graph; §9) — is why this review recommends *extending* the brain-inspired lineage rather than either alternative: it starts already satisfying the requirements that are structural dead-ends for the other two, and its remaining gaps are reachable by grafting rather than re-founding.
...
## 8. Synthesis — The Gaps, and the Direction This Justifies

Pulling the review together: we have a validated blueprint for representing and navigating a *known* structural graph (TEM and its unifications), but no trained architecture that closes the several gaps a full reasoning model needs — **inferring latent transformation rules (Type-2 / ARC-AGI edges) while retaining cross-environment meta-graph transfer, co-discovering its own vocabulary, bootstrapping goals without an external verifier, and grounding an objectness prior.** These are *interacting* gaps, not a ranked list; the review's job is to justify the brain-inspired approach and specify the characteristics of the systems that must fill them, leaving the concrete modeling to the reader (§8.1).

Three of these gaps are localized with unusual precision by convergent evidence:
...
- TEM's own limitation: the action vocabulary `a_t` is always *externally given*; TEM is forward-only and cannot infer transformations (Block 3A).
...
Consolidating the scattered checklists (§1, §4.5, §4.6, §5) into one matrix. Rows are the machinery the latent-graph-discovery problem *requires*; columns are the leading model classes. Several rows are empty across the existing classes — **latent transformation inference (Block 3A)**, hierarchical rule-of-rules control, planning-as-inference, and (for all but the solver) per-instance correctness. The point is not one decisive cell but that *no existing class covers the full requirement set*, whereas the brain-inspired core (§5.8) covers most of it by construction and reaches the rest by grafting. The bottom two rows show the two camps are *complementary*, not rival: the solver uniquely owns per-instance correctness; everything else uniquely owns verifier-free operation.
...
| Planning-as-inference, not rollout (§5.7) | ❌ | ~ | ❌ | ext | ❌ | ❌ˢ |
| Objectness / Core-Knowledge front-end (§3, §8.3) | ❌ | ~ | ✅ | n/a | ❌ | ❌ᴰ |
| Verifier-free operation (ARC-AGI-3, open discovery) | ✅ | ✅ | ✅ | ❌ | ✅ | ✅ |
| Per-instance correctness guarantee (§4.6) | ❌ | ❌ | ❌ | ✅ | ❌ | ❌ |

*Legend:* ✅ satisfies · ~ partial · ext externalized to human infrastructure · n/a not applicable to substrate · ❌ absent. *Superscripts:* ᴹ fast Hebbian M only, no slow learned W · ᵍ vocabulary *given* (mathlib/DSL), not inferred → Tao ceiling (§4.6) · ˢ absent in *base* TEM, supplied via the Spacetime Attractor planner (§5.7) in the recommended build · ᴰ absent in *base* TEM, supplied via a DINO/JEPA SSL front-end (Blocks 1A/2A, §4.5) in the recommended build.

**Wider approaches roster.** The matrix above scores the six model classes that map cleanly onto every LGD requirement. For completeness, the engineered HC/PFC-memory line and the neuro-symbolic/program-synthesis family are positioned here against *one decisive requirement* — latent transformation inference with cross-environment transfer (Block 3A, source 2) — since that is where each ultimately stalls (it is one of the gap set above, not the sole yardstick):

| Approach class | Key mechanism | Position on the decisive row (Block 3A / source 2) |
|---|---|---|
| **DNC** | LSTM controller + external read-write memory; 98.8% graph traversal | Engineered HC/PFC analog with an M-like store, but *no* factorized `g/x`, no slow-W meta-graph, vocabulary given → no transformation inference |
| **Vector-HaSH** | Grid *scaffold* + content factorization; exponential capacity | Closest existing memory model to TEM's factorization and a strong Block-2A substrate candidate — but binds a *given* code, learns no transformation vocabulary |
| **MLC** | Episodic meta-learning transformer (Lake & Baroni lineage) | Demonstrates the *meta-training route* to the W/M split, but no explicit `g/x` factorization and hits a 100% error ceiling on productivity/structural — no 3A |
| **CSCG** | Clone cells + Bayesian EM | Solves observation aliasing (source 3) cleanly, but is fast/local with *no* cross-environment W transfer → no meta-graph, no 3A |
| **Program synthesis / DSL search** (DreamCoder, icecuber; CoreThink) | Induction over a hand/library-built DSL; the actual ARC-AGI-1 winners | The neuro-symbolic sibling of the solver (§4.6): vocabulary *given or library-learned*, so it hits the same source-2 wall; DreamCoder's library learning is the closest existing stab at vocabulary co-discovery, but domain-specific. **CoreThink** (Das et al. 2025) is a recent ARC-AGI-2 instance (30.8% pass@2): 22 hand-curated patterns + connected-components objectness, *both given* — sidesteps Gaps #3/#9, and its ablation confirms the gains are structural bias, not scaling |

Every row lands on the same verdict as the main matrix: the Block-3A cell — latent transformation inference *with* cross-environment transfer — stays empty across the entire existing-approaches landscape. That vacancy is one of §9's open problems and a genuine frontier shared with the solver literature (§4.6) — but it is one of several gaps a full model must close (goal bootstrapping, vocabulary transfer, objectness, multi-level meta-graph), not the review's single thesis.

### 8.1 The recommended architecture

A factorized, two-timescale world model in the TEM lineage, extended to close the Type-2 gap. The functional decomposition and a defensible build order:
...
- **Why a Transformation Inferrer is in the block set.** It is precisely the missing inverse of the one operation TEM already does well (path integration), it directly targets an empirically-isolated bottleneck (edge/vocabulary co-discovery; Joffe & Eliasmith 2025), and it is the prerequisite for Type-2 tasks (ARC-AGI, analogy, rule induction). It earns a place among the blocks for that reason — but it is one required component alongside the others (goal bootstrapping, vocabulary transfer, hierarchical stack, objectness front-end), not the system's sole crux; which to build first is left to the modeling stage.
...
2. **Vocabulary co-discovery with cross-environment transfer** — no model does both simultaneously (DreamCoder's library learning and LAPA both do vocabulary co-discovery but domain-specifically — DreamCoder is the closest existing stab, §8.0; VSA does binding but learns no W). This is the *same wall the solver stack hits from the formal side* — Tao's "discovery modulo expertise" ceiling and FrontierMath Tier-4's single-digit scores (§4.6) — making it the shared frontier of both literatures and the natural site of a novelty claim. **Neuroscience supplies the *selection* half, not the *invention* half:** contextual inference (COIN/MOSAIC; §5.7) infers which action→outcome model applies from a *learned* repertoire, and BCI neural-constraints work (Sadtler et al. 2014; Golub et al. 2018) shows the fast timescale is *confined to a preserved repertoire* — no fast operator invention has been demonstrated in or out of the intrinsic manifold. So the brain buys vocabulary *selection*; genuine vocabulary *invention* is the hard core that stays open, and — as a testable consequence — the pretraining corpus must cover the operator space near-exhaustively rather than merely span a rich manifold.
...
The literature synthesized here supports a clear and defensible direction. The target capability (abstract reasoning) reduces to a single well-posed problem (latent graph discovery); the reason scale has not solved it is structural (missing factorization → unreachable manifold), not empirical; and the brain supplies a convergent, increasingly formalized blueprint (factorized two-timescale world model + hierarchical PFC control + planning-as-inference + neuromodulatory goal bootstrapping). The field's frontier is a *set* of interacting gaps — inferring latent transformation rules while retaining meta-graph transfer, co-discovering vocabulary, bootstrapping goals without an external verifier, and grounding objectness — and the direction the evidence justifies is a factorized, two-timescale, brain-faithful world model that runs the coupled Navigator⇄Strategist loop and grows to cover them. Every characteristic (factorization, two timescales, path integration, hierarchical control, planning-as-inference, goal bootstrapping) follows from the brain evidence rather than convenience. How to compose them into a concrete model — and how simple or faithful to make the first version — is left to the reader. This is the rationale on which a concrete method can now be built.

---

## Implications

- The contribution is the *whole* factorized, two-timescale, brain-faithful world model and the coupled Navigator⇄Strategist loop that drives it — not any single block. The concrete instantiation and the order in which gaps are attacked are modeling decisions left to the reader, best approached from a simple core built on the core principles, then learned from and remodeled iteratively.
...
- Is set-attention over Δg sufficient for *multi-step* transformation chains (A→B→C), or is iterative inference required?
