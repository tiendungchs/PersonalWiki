...
- **TANs as catastrophic interference gate:** Cholinergic tonically-active neurons (TANs) learn to pause in rewarding contexts, releasing MSNs from inhibition and enabling corticostriatal RL. During extinction, TANs stop pausing → MSNs re-inhibited → learned weights protected. Fast reacquisition arises because intact learned weights re-emerge when TANs re-learn to pause — a second-order gate on the BG learning gate itself, implementing the same interference-protection the CLS two-timescale split achieves at the cortical level.
- **BG-WM three-architecture debate:** (a) BG gates thalamo-cortical reverberation (Monchi; FROST); (b) BG gates cortico-cortical loop then steps back — PBWM (Frank); (c) BG inside the maintenance loop (Schroll). Dissociation: (b) predicts thalamic lesion spares WM maintenance; (c) predicts BG lesion directly impairs maintenance, not just gating.
- **COVIS dual-system categorization:** Hypothesis-testing system (rapid, verbal, BG for WM gating + rule switching) and procedural system (slow dopamine RL in BG for stimulus-response associations). Dissociated by feedback delay: delay impairs procedural but not hypothesis-testing. This is the precursor dual-BG-loop architecture to formal meta-RL.
- **SPEED automaticity — BG trains then withdraws:** BG direct pathway trains cortical association→premotor connections via dopamine RL until consistent premotor firing triggers Hebbian LTP on the direct cortico-cortical route. Once strong enough, BG is no longer required for execution. Confirmed: GPi inactivation (Desmurget & Turner 2010) disrupts movement kinematics but not learned sequence knowledge — the compiled cortical route survives BG disconnection.

**Limitations:** No model simultaneously accounts for cognitive and motor BG function; direct-pathway-only models cannot reproduce full inhibitory/braking dynamics; 2013 predates deep-RL accounts of BG and modern SNN implementations.
