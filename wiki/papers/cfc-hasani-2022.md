...
Hasani, Lechner, Amini et al., *Nature Machine Intelligence* 4, 1112–1119 (2022). Derives a provably tight closed-form approximation of the LTC (Liquid Time-Constant) ODE, eliminating the need for numerical differential equation solvers while preserving expressivity.

---

- The integral `∫₀ᵗ f(I(s))ds` that appears as an exponent in LTC (Liquid Time-Constant) dynamics can be approximated as `f(I(t))·t`; the error is bounded by `|x₀-A|·e^{-w_τ·t}` — decaying exponentially, so the approximation improves over time (sharpness result: bound is tight).
