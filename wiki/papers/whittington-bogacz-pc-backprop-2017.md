...
# Whittington & Bogacz 2017 — PC Approximates Backprop
...
- **Three convergence conditions.** PC = exact backprop when: (a) the network already predicts all training samples perfectly; (b) predictions are close to targets (near-perfect learning); (c) output-layer variance parameter σ_out → 0, which reduces the perturbative effect of clamping the output. In general nonlinear networks, this is an approximation with finite bias.

- **Bidirectional association.** With equal noise parameters on input and output arms, PC learns the first principal component of the joint distribution — simultaneously supporting forward (input→output) and reverse (output→input) inference. Setting σ_out ≪ σ_in recovers backprop (forward regression); setting σ_in ≪ σ_out gives inverse regression. Standard backprop provides no backward inference.

- **MNIST validation.** A 784–600–600–10 PC network achieves ~1.7–1.8% validation error, matching a standard ANN with backprop. The σ_out → 0 case tracks the ANN learning curve most closely; both eventually reach 0% training error.
...
- **[[wiki/concepts/predictive-coding.md]]** — full derivation of the PC energy function, two-population architecture, and how Whittington & Bogacz 2017 establishes the bias floor for the PC credit assignment approximation.
- **[[wiki/concepts/credit-assignment.md]]** — places this result in the bias–variance taxonomy: PC occupies medium-bias/medium-variance; convergence to exact backprop at the energy minimum defines the minimum achievable bias.
...
- **[[wiki/concepts/two-learning-timescales.md]]** — the PC weight rule is the candidate biologically plausible slow-W learning algorithm; this paper provides the formal ground for using PC as the outer-loop credit assignment rule in architectures that prohibit global backward passes.
