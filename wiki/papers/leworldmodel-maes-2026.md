...
- **48× faster planning than foundation-model-based world models.** ViT-Tiny encoder (5M) + transformer predictor (10M) = 15M parameters total. Encoding observations with ~200× fewer tokens than DINO-WM (DINOv2 frozen ViT-L) enables planning in under 1 second while remaining competitive on Push-T (96% SR (Successor Representation) vs. DINO-WM 92%).
