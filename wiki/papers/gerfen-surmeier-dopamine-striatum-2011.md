...
Review of two decades of evidence for D1/D2 receptor segregation in striatal direct and indirect pathways, culminating in cell-type-specific physiological and plasticity characterization enabled by BAC transgenic reporter mice.

---

## Five Key Computational Insights

- **Opponent plasticity from a single DA event.** D1 (direct SPN) → Gs → PKA → Cav1 Ca²⁺ ↑, K⁺ ↓, AMPA/NMDA surface expression ↑ → LTP + excitability ↑. D2 (indirect SPN) → Gi → cAMP ↓, Cav1 ↓, K⁺ ↑, endocannabinoid production → retrograde CB1 → Glu release ↓ → LTD + excitability ↓. A single phasic DA burst writes opposite plasticity rules to both populations simultaneously — credit assignment is inherently bidirectional, not scalar.
...
- **Cholinergic temporal gating.** Thalamic salience inputs drive cholinergic interneurons through burst-pause firing. Burst: M2 presynaptic suppression of corticostriatal Glu release. Pause: M1 postsynaptic on D2 SPNs closes Kir2 → ~1-second window of enhanced indirect-pathway dendritic excitability. DA (D2 on cholinergic interneurons) gates the pacemaker — baseline DA suppresses the burst-pause; DA drops allow thalamic salience to redirect processing through the NoGo pathway.

- **Parkinson's disease as natural experiment.** DA depletion reverses plasticity bias in both pathways simultaneously: D1 loss → direct SPNs shift to LTD bias; D2 loss → indirect SPNs gain LTP capacity. Both shifts favor indirect pathway dominance → action suppression locked in. Confirmed by optogenetic selective activation (Kravitz et al. 2010). Implication: opponent credit assignment is architecturally necessary — lesioning one side is not compensated by the other.
...
- Distal SPN dendritic integration is almost completely unknown; D1/D2 receptors are most dense in distal dendrites yet all patch-clamp work is somatic or proximal.

---

## Links

- [[wiki/entities/basal-ganglia.md]] — full anatomical and functional account of the BG circuit
- [[wiki/concepts/neuromodulation.md]] — D1/D2 cellular cascades ground the "DA = TD error" metaparameter in specific receptor biochemistry
- [[wiki/concepts/meta-learning.md]] — PVLV cellular substrate: opponent LTP/LTD is the synaptic plasticity step underlying BG slow outer loop
- [[wiki/papers/pbwm-oreilly-frank-2006.md]] — PBWM model whose per-stripe DA signal this paper grounds at the receptor level
