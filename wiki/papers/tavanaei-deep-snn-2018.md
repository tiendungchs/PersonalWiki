...
- **STDP ≈ Bayesian EM:** With winner-take-all lateral inhibition, STDP with Poisson inputs approximates online Expectation-Maximization for a multinomial mixture distribution (Nessler et al. 2009/2013); the WTA (Winner-Take-All) spike output is an E-step sample from the posterior; the STDP update is the M-step — unsupervised generative model learning from local spike-timing rules alone.
...
- **LSNN (Bellec et al. 2018):** A reservoir of ALIF (Adaptive Leaky Integrate-and-Fire) spiking neurons trained with BPTT + pseudo-derivatives achieves LSTM-comparable accuracy on sequential MNIST and TIMIT without explicit gating; the slow adaptation current in ALIF (Adaptive Leaky Integrate-and-Fire) (τ_a ≫ τ_m) plays the role of LSTM cell state — sequential memory from neuronal dynamics rather than architectural machinery.
