...
- **JEPA is not Predictive Coding.** PC (Predictive Coding) operates on a single moment-to-moment sensory input and converges via hierarchical recurrent error propagation — inference does not require a second input. JEPA requires two inputs (x and y) to compute z as the relationship between them; it cannot perform the same online single-observation iterative inference. Hierarchical JEPA stacking (higher level feeds lower) also does not cleanly map to PC's top-down error feedback hierarchy; it is unclear how to make a higher-level JEPA provide the z for a lower-level one.
...
- **VICReg approximates mutual information via variance and covariance.** The high-level idea: maximize MI between encoder input and output (prevents collapse); minimize information in z (prevents cheating). Computing MI exactly requires KL (Kullback-Leibler) divergence — intractable. VICReg's V (variance hinge) + C (off-diagonal covariance Frobenius) are tractable per-batch proxies for this MI objective. (The specific per-branch independence design and its Barlow Twins comparison come from the VICReg paper, not this review — see [[wiki/papers/vicreg-bardes-2022.md]].)
