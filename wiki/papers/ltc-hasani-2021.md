...
- **Biologically derived ODE**: LTC equation `dx/dt = -x(t)/τ + f(x,I,t,θ)·(A-x)` is derived from the conductance-based cable equation for nonspiking neurons — specifically C. elegans circuit models, not merely analogically inspired. The same mathematical form governs synaptic current summation: `dv/dt = -gl·v + f(v,I)·(A-v)`, where `1/τ ↔ gl` (leakage conductance).
...
- **DCM structural resemblance**: LTC semantics resemble Friston's bilinear Dynamic Causal Models (`dx/dt = (A + I(t)B)x + CI`), used to model fMRI time-series. ODE structure inherently possesses causal ordering — LTCs are a candidate class of causal temporal generative models.
