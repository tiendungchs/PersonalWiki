...
**Citation:** Marcus Hutter, preprint, Munich 2000. Foundational paper introducing the AIXI (AI with (X) induction (I)) model.
...
- **Horizon problem: formal counterpart of WM timescale design.** AIXI (AI with (X) induction (I))'s output is acutely sensitive to planning horizon m_k. Greedy (m_k = k) fails at chess. Infinite (m_k = ∞) causes pathological defer-forever in environments with long-range dependencies (the "heaven/hell" example: one action in cycle 1 determines all future credits; no unbiased policy can avoid catastrophic loss for some µ). Factorizable µ (independent episodes — the IID assumption) is the exact class where m_k → ∞ is safe.

- **Proposition 2 connection (Choi 2026 [[wiki/concepts/intelligence-density.md]]):** Legg-Hutter $\Upsilon(\pi)$ is monotonically increasing in Intelligence Density $\mathcal{I}(\pi) = \log_2 N(\pi) / C(\pi)$ under evolutionary selection with a fixed environment and survival-based reward. This positions AIXI (AI with (X) induction (I)) as a special case of the $\mathcal{I}$-framework requiring environment + reward specification; $\mathcal{I}$ generalises Legg-Hutter to substrate-independent settings without task or reward.
...
- AIXI (AI with (X) induction (I)) itself is uncomputable; AIξ^{t̃,l̃} has exponential overhead 2^{l̃}, impractical for realistic l̃.
...
- **[[wiki/concepts/information-theory.md]]** — AIXI (AI with (X) induction (I)) extends the Solomonoff predictor (passive, sequence prediction) to the active decision-making case; ξ^AI conditioned on action history; the passive/active boundary is the formal limit of the compression-prediction equivalence.
- **[[wiki/concepts/latent-graph-discovery.md]]** — AIXI (AI with (X) induction (I)) maintains a mixture over all computable latent graphs (all environments); solving all four hardness sources simultaneously; uncomputable, so all feasible architectures are bounded-program approximations to it.
- **[[wiki/concepts/meta-learning.md]]** — AIXI (AI with (X) induction (I)) is the theoretical ceiling of meta-learning: Solomonoff prior = ideal slow outer loop (never learned, defined by K complexity); expectimax = ideal fast inner loop (pure inference, no gradient updates).
- **[[wiki/concepts/abstract-reasoning.md]]** — AIXI (AI with (X) induction (I)) provides the formal definition of general intelligence via the intelligence order relation ⪰; the three Lake et al. ingredients are engineering routes to approximating the Kolmogorov simplicity prior.
...
- **[[wiki/concepts/predictive-coding.md]]** — active inference (Friston) is a local approximation of AIXI (AI with (X) induction (I))'s action component: action minimizes variational free energy rather than expectimax credit, both reducing to the same behavior for passively predictable environments.
- **[[wiki/concepts/two-learning-timescales.md]]** — the W/M split approximates the AIXI (AI with (X) induction (I)) Solomonoff prior (slow W) + expectimax inference (fast M); factorizable µ is the exact mathematical condition under which the two-timescale separation is lossless.
- **[[wiki/concepts/intelligence-density.md]]** — Choi 2026 Proposition 2 shows Legg-Hutter $\Upsilon(\pi)$ is monotonically increasing in $\mathcal{I}(\pi)$; AIXI (AI with (X) induction (I)) is a special case of the $\mathcal{I}$-framework where environment and reward are specified; $\mathcal{I}$ generalises AIXI (AI with (X) induction (I))'s intelligence order relation to substrate-independent settings without task or reward.
