...
# Bogacz & Gurney 2007 — BG as Optimal Decision Maker
...
- **MSPRT circuit decomposition:** The cortex–BG circuit implements the Multihypothesis Sequential Probability Ratio Test (MSPRT) in the proficient phase. Direct pathway (D1 striatum → GPi/SNr) computes the negated salience term −yi(T). STN receives diffuse cortical excitation and fires exponentially (exp transfer function); GP log-compresses STN via inhibitory feedback; their combined output is S(T) = ln(Σ_j exp(yj(T))). BG output: OUT_i = −yi(T) + S(T) = −ln P(H_i | evidence). Selection fires by disinhibition when any OUT_i drops below threshold — equivalent to selecting the action with maximum Bayesian posterior probability. This is not winner-take-all but Bayesian log-posterior selection.

- **Biologically validated transfer functions:** MSPRT predicts STN must fire as exp(input) and GP must log-compress STN activity. Exponential STN I/O confirmed in Hallworth et al. 2003 and Wilson et al. 2004 (current-injection recordings). GP fires approximately linearly in STN input — which equals log-compression for large N (ln(linear · N) dominates) — confirmed Nambu & Llinas 1994. Both predictions were derived from the algorithm *before* comparison with physiology, then confirmed post-derivation.

- **Proficient vs. learning phase dissociation:** D2 striatum → GPe → STN serves MSPRT normalization (computing S(T)) during proficient execution. During the learning phase, the same D2 pathway serves punishment blocking (blocking actions associated with negative outcomes, Frank et al. 2004). Same anatomy, context-determined mode. The paper restricts to proficient phase; the full BG account requires MSPRT (execution) + PVLV/RL (learning) as complementary modes. The transition mechanism between modes is not specified.
...
- **Parametric robustness and cortical compatibility:** The MSPRT model is robust to miscalibrated cortical gain g: overestimation converges to MSPRT_b (a second asymptotically optimal test); underestimation converges to UM model (mutual inhibition); never worse than UM for any g. If cortex itself implements UM-style competition (inhibitory integrators), the BG output is mathematically unchanged — the Bayesian log-posterior is invariant to upstream cortical competition mode.

---

**Limitations:** Restricted to proficient phase; does not specify how BG transitions between MSPRT execution mode and RL learning mode, or what DA state triggers each. Assumes localist (one-per-action channel) action representations; extension to distributed representations requires additional machinery noted as future work. Does not address simultaneous cognitive and motor loop selection — the cognitive-motor integration gap (Helie et al. 2013) is unaddressed.
