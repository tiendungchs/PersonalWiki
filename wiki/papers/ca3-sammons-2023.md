...
**Method:** Combined 3D EM (ATUM-multiSEM, 965×808×62 µm at 4 nm resolution; 7 reconstructed axons, 1,062 annotated synapses) with multipatch electrophysiology (up to 8 simultaneous cells; 1,172 tested pairs across 52 mice) and spiking network simulations (LIF + log-normal weights + inhibitory STDP).

---

## Key Computational Insights

- **CA3 connectivity is ~9–11%**, not <1%: EM finds 11.2 ± 2.7% within 50 µm; electrophysiology finds 8.8% (103/1,172 pairs). Both methods agree across independent labs — ~10× above Guzman et al. 2016's estimate.
...
- EM from a single P31 male mouse; 62 µm slice thickness means truncated axons — measured connectivity is likely a *lower bound*.
