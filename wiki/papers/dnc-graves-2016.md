...
- **External memory as a differentiable RAM:** The DNC separates computation (LSTM controller) from storage (N×W memory matrix). The controller emits interface vectors that parameterize soft read/write operations via attention weightings. Because behavior is independent of memory size, a DNC trained with 256 memory locations can be upgraded to a larger memory without retraining and performance scales accordingly — confirming that memory is genuinely external to the learned function.
...
- **LSTM fails completely on graph tasks; DNC does not:** On graph traversal trained with curriculum learning, the best LSTM found in extensive hyperparameter search reached only 37% accuracy after ~2M training examples; DNC reached 98.8% after ~1M. Shortest-path (55.3%) and inference (81.8%) tasks show the same gap. This is the strongest empirical demonstration in the wiki that external read-write memory is *necessary* — not merely helpful — for structured graph reasoning.

- **Emergent prospective planning:** In the Mini-SHRDLU block-puzzle RL task, DNC wrote its intended action plan to memory at goal-presentation time, many steps before execution. A logistic regression decoder applied to those memory locations predicted the first action with 89% accuracy (vs. 17% chance), confirming a write-then-read prospective planning strategy not explicitly trained.

- **Explicit hippocampal parallels:** The authors directly map DNC mechanisms to HC biology: fast one-shot memory modification ≈ CA3/CA1 associative LTP; usage-based allocation + sparse weightings ≈ DG neurogenesis and representational sparsity; temporal link matrix ≈ temporal context model for human free-recall order (Howard & Kahana 2002). The parallel is structural: DNC is an engineered implementation of the computational functions the hippocampus performs.
...
- Controller capacity remains fixed — meta-graph representation (slow W) is still the controller's LSTM weights; DNC only externalizes the instance-graph (fast M); cross-environment generalization is not addressed.
...
→ **[[wiki/concepts/associative-memory.md]]** — Content-based lookup as differentiable CAM  
→ **[[wiki/concepts/sequence-memory.md]]** — Temporal link matrix as one-shot sequence encoder  
→ **[[wiki/concepts/latent-graph-discovery.md]]** — DNC as the first verified graph-traversal solution  
