...
created: 2026-06-20
updated: 2026-07-17
sources: [Modulation of striatal projection systems by dopamine, Exploring the cognitive and motor functions of the basal ganglia an integrative review of computational cognitive neuroscience models, Basal_Ganglia_and_Cortex, Frontal Networks for Learning and Executing Arbitrary Stimulus-Response Associations]
related: [wiki/concepts/arbitrary-mapping.md, wiki/concepts/neuromodulation.md, wiki/concepts/meta-learning.md, wiki/concepts/working-memory.md, wiki/concepts/two-learning-timescales.md, wiki/concepts/cognitive-control.md, wiki/concepts/sequence-memory.md, wiki/entities/prefrontal-cortex.md, wiki/papers/pbwm-oreilly-frank-2006.md, wiki/papers/metalearning-neuromodulation-doya-2002.md, wiki/papers/pfc-meta-rl-wang-2018.md, wiki/papers/gerfen-surmeier-dopamine-striatum-2011.md, wiki/papers/helie-ccn-bg-2013.md, wiki/papers/bogacz-gurney-bg-msprt-2007.md, wiki/papers/long-sequence-hopfield-chaudhry-2023.md, wiki/entities/covis-model.md, wiki/papers/frontal-cortex-abstract-rules-badre2010.md, wiki/papers/boettiger-desposito-sr-rule-learning-2005.md, wiki/papers/collins-frank-cts-task-set-2013.md, wiki/concepts/contextual-inference.md, wiki/concepts/hierarchical-reinforcement-learning.md]
---
...
**Striatum does not disengage when the cortical inferrer does (Boettiger & D'Esposito 2005 [[wiki/papers/boettiger-desposito-sr-rule-learning-2005.md]]).** In human categorical S-R rule learning, right anterior caudate is learning-selective (novel > familiar, and novel > a no-rule control matched on novelty/uncertainty/error rate, p<0.01) — but unlike dlPFC and SMA, which decay to baseline as accuracy rises, **striatal activity does not decay** (it trends upward, matching Brasted & Wise monkey recordings during conditional visuomotor learning). The same dissociation appears in Badre 2010's late striatal activation for hierarchical rules. **Reading:** the BG's contribution is not a transient bootstrap that shuts off once a rule is known — the inference side disengages while valuation continues, consistent with the SPEED account in which BG keeps training cortico-cortical weights across the whole acquisition arc. A design that treats the RL gate as scaffolding to be removed after acquisition gets this backwards.

**Striatum as an early, post-cue module in arbitrary mapping (Schultz et al., via Wise & Murray 2000 [[wiki/papers/wise-murray-arbitrary-mapping-2000.md]]):** ~30% of striatal neurons differentiate novel from familiar cue→action mappings. Two asymmetries sharpen the BG's role: (a) frontal areas show a *higher prevalence* of learning-related activity change than striatum; (b) striatal learning-related change concentrates immediately after the **cue**, unlike premotor areas where it appears across task events — the same pattern found for arbitrary auditory-motor mappings in rats. Reading: the striatum acts at the *earliest phase of a structured event sequence*, consistent with the putamen→cortex RL-gate direction above (it opens the update window at cue onset rather than carrying the mapping itself). Thalamic lesions intended to disrupt BG→frontal flow profoundly impair arbitrary mapping, confirming the BG contribution is routed through frontal cortex. See [[wiki/concepts/arbitrary-mapping.md]].

---

## Nested Corticostriatal Loops that *Create* Structure (Collins & Frank 2013)

The stripe-gating account above selects among **pre-existing** representations. Collins & Frank 2013 ([[wiki/papers/collins-frank-cts-task-set-2013.md]]) extend PBWM one crucial step — the same Go/NoGo/DA machinery is used to **create and cluster latent task-set (TS) reps**, not just admit known ones:

| Loop | Gates | Learns |
|---|---|---|
| **Anterior (PFC-BG)** | an abstract **TS stripe** in response to *context* | which context→TS mapping via DA-RL; a "blank" stripe = **a newly created task-set** |
| **Posterior (PMC-BG)** | the **motor action** given stimulus × gated TS | S-A-outcome weights *within* the selected TS |

The gated PFC TS stripe multiplexes with the stimulus in parietal cortex and becomes the input to the posterior loop — so a task-set is literally "action selection contextualized by a self-invented latent state." Two properties distinguish this from ordinary WM gating:

- **No a-priori correct target.** For motor gating there is a rewarded action to learn toward; for the PFC TS stripe there is none — the network chooses *which* stripe represents *which* latent state, i.e. it performs unsupervised structure discovery under DA-RL. This is the BG mechanism for the query's "which subgoals/rules are worth admitting."
- **Create-or-reuse is a gating decision.** Encountering a novel context, the anterior loop either gates an existing (popular) TS stripe or gates a fresh blank one — the neural analog of the C-TS Dirichlet-process choice (reuse ∝ popularity vs. create ∝ concentration α). Simulated: ~70% of nets reuse the correct TS1 stripe for a transfer context; ~90% correctly recruit a blank stripe for a genuinely new one.

**Cross-loop diagonal projection = the RT cost of structure.** TS-level conflict (co-active anterior PFC stripes before the TS is resolved) excites the motor-loop STN via a diagonal PFC→STN projection, raising the motor decision threshold (Hold) until the task-set settles — switch-cost reaction times, and reduced cross-task interference because the effective state space is expanded per TS.

**Design implication:** this is the concrete corticostriatal instantiation of *create-or-reuse* subgoal/rule gating for Block 3B/3C. A blank-stripe recruitment operator, gated by the same DA-RL that trains selection, gives an architecture the ability to grow its own latent-state vocabulary — but note it grows only *representational* task-sets (context clusters), not temporally-extended options. See [[wiki/queries/hrl-goal-decomposition-coverage.md]].
...
- **[[wiki/concepts/arbitrary-mapping.md]]** — the striatum's contribution to binding arbitrary cue→action mappings is early and cue-locked (~30% of neurons differentiate novel from familiar mappings, changes concentrated immediately post-cue), and is routed through frontal cortex — BG→thalamus lesions impair the behavior; this is the arbitrary-mapping instance of the putamen-as-RL-gate role.
- **[[wiki/papers/wise-murray-arbitrary-mapping-2000.md]]** — source for the striatal novel-vs-familiar mapping result, the cue-locked concentration of learning-related change, and the thalamic-lesion evidence that BG acts on arbitrary mapping via the frontal lobe.
- **[[wiki/papers/boettiger-desposito-sr-rule-learning-2005.md]]** — source for the human striatal learning-selectivity-without-decay result: caudate distinguishes novel from familiar *and* from no-rule mappings, but does not go offline as the cortical inferrer does — the asymmetry that separates the inference side from the valuation side of the binding circuit.
...
- **[[wiki/papers/collins-frank-cts-task-set-2013.md]]** — source for the nested corticostriatal loops that *create and cluster* task-set stripes (not just gate known ones): anterior PFC-BG loop invents latent-state stripes with no a-priori correct target, posterior PMC-BG loop selects actions within the gated TS; create-or-reuse of a blank stripe is the neural analog of the Dirichlet-process concentration choice; diagonal PFC→STN projection gives the switch-cost RT.
- **[[wiki/concepts/contextual-inference.md]]** — the Collins & Frank create-or-reuse stripe gate is the corticostriatal, cognitive-control-side ancestor of COIN's HDP context inference: both grow an unbounded latent-state repertoire by a concentration-weighted new-vs-reuse decision; the BG gate is the mechanism the contextual-inference concept says the COIN paper leaves unimplemented.
- **[[wiki/concepts/hierarchical-reinforcement-learning.md]]** — in HRL the dorsolateral striatum implements *option-specific policies* (frontal option reps gate which S-R pathway fires), and subtask-boundary phasic DA gates option/WM updates; the C-TS create-or-reuse stripe gate is the antidote to HRL negative transfer from a fixed option library.
