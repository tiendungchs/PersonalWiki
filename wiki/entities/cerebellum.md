...
| Cell type | SDM (Sparse Distributed Memory) role | Approx. count (cat) | Connectivity |
|---|---|---|---|
...
| SDM (Sparse Distributed Memory) component | Cerebellar analog |
|---|---|
...
| Model | Key claims | SDM (Sparse Distributed Memory) correspondence |
|---|---|---|
...
| **Albus (1971) / CMAC** | Systematic cubicle-tiling address structure; continuous input variables; climbing fiber = error signal; LTD (Long-Term Depression) (not LTP) at parallel→Purkinje | SDM (Sparse Distributed Memory) over continuous multidimensional state space; extends SDM (Sparse Distributed Memory) to sensorimotor inputs |
| **Kanerva (1993)** | Unifies Marr/Albus within SDM (Sparse Distributed Memory) framework; hyperplane-design variant; Golgi cells as p_opt regulator | Formal proof that cerebellar cortex is a biological SDM (Sparse Distributed Memory) |

---

## Key Implication: SDM (Sparse Distributed Memory) as a Domain-General Primitive
...
| **Cerebellar cortex** | Supervised LTD (Long-Term Depression) via climbing fiber error | Motor learning: context → feedforward correction | Inferior olive error |
...
- Cerebellar SDM (Sparse Distributed Memory) implements feedforward motor correction, not iterative pattern completion — Purkinje cells are inhibitory to deep cerebellar nuclei, creating a one-shot output rather than recurrent attractor dynamics.
...
- **[[wiki/entities/sdm-model.md]]** — cerebellum is the biological instantiation of the SDM (Sparse Distributed Memory) hyperplane design; the Golgi cell dynamic threshold is the biological mechanism for maintaining p near p_opt that the theoretical analysis requires.
- **[[wiki/entities/hippocampal-entorhinal-system.md]]** — DG→CA3 is the second biological SDM (Sparse Distributed Memory) instantiation; both use k=3–5 sparse address connections per hard location; the difference is write supervision (unsupervised HC vs. climbing-fiber-supervised cerebellum), not circuit topology.
- **[[wiki/concepts/sparse-distributed-representations.md]]** — cerebellar granule cell expansion (N→M with M >> N) is the same random high-dimensional projection SDR (Sparse Distributed Representations) theory formalizes; ~1% active granule cells sit in the regime where inter-pattern interference is negligible.
- **[[wiki/concepts/pattern-separation.md]]** — DG (Dentate Gyrus) granule cells and cerebellar granule cells share the same expansion + k=3–5 sparse address design; both use threshold regulation (Golgi cells / competitive inhibition) to enforce p_opt; convergent architecture in two circuits.
- **[[wiki/concepts/associative-memory.md]]** — parallel fiber→Purkinje LTD (Long-Term Depression) is the supervised analog of Hebbian C-matrix updates; climbing fiber one-shot error drive mirrors the outer-product write in unsupervised SDM.
...
- **[[wiki/papers/kanerva-sdm-1993.md]]** — primary source for the SDM (Sparse Distributed Memory) cerebellar mapping; Marr (1969) and Albus (1971) as the original models; quantitative cell-count correspondence.
