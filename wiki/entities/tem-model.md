...
- **[[wiki/queries/operator-collapse-in-fused-structural-codes.md]]** — measures what breaks in a TEM-lineage world model that keeps path integration but drops the g/x factorization and the prior loss: the action operators collapse onto the identity while pixel prediction stays at 98.5%. Evidence that TEM's `lx_gt` (predict x from the prior `g` alone) is not incidental bookkeeping but the term that makes the structural code carry structure.
