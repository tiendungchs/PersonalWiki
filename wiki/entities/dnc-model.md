...
| **Usage vector u** | u∈[0,1]^N; tracks cumulative use; free gates release locations after read | DG representational sparsity |
| **Allocation weighting a** | Picks least-used location for new writes (differentiable free list) | DG adult neurogenesis (new coding units) |
...
**Planning decode (Mini-SHRDLU):** First-action classification accuracy from memory contents at goal-write time = **89%** (vs. 17% chance baseline) — DNC committed to a plan in memory many steps before execution.
...
- **No cross-episode memory persistence:** DNC memory is reset between episodes; long-term consolidation (HC → neocortex) is absent — fast-M only.
...
- **[[wiki/entities/hippocampal-entorhinal-system.md]]** — DNC is the clearest engineered instantiation of the PFC-queries-HC planning architecture: controller=PFC, memory matrix=HC, content lookup=CA3 pattern completion, temporal links=temporal context model for free recall, allocation=DG neurogenesis; three out of three HC mechanisms have direct DNC analogs.
- **[[wiki/concepts/associative-memory.md]]** — DNC content lookup generalizes Hopfield's binary CAM to differentiable soft attention over a continuous memory matrix; DNC demonstrates that external CAM provides graph traversal capability inaccessible to weight-bound associative memory (LSTM: 37%).
...
- **[[wiki/concepts/two-learning-timescales.md]]** — DNC mechanically instantiates the W/M split: controller LSTM weights = slow W (trained across episodes by BPTT); external memory writes = fast M (written within episode by soft attention with no weight gradient needed for the write itself).
