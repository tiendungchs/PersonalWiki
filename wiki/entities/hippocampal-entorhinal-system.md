...
| Discrete, non-metric, **learned via experienced transitions** | **Yes** | Implicit graph from random walks (Garvert et al. 2017) |
| Discrete-sampled, **2D-embeddable + inferentially traversed for a decision** | **Yes** (HC+EC map; EC+mPFC grid) | Social hierarchy reconstructed from binary comparisons (Park et al. 2021) |
| Taxonomic-semantic similarity, **explicit + lifetime-acquired** | **Yes** (posterior HC; passive/task-irrelevant) | Object category space, mapped alongside a transition graph (Zheng et al. 2024) |
| Declarative associative, **known without traversal** | **No** | Social network traversal (Kumaran & Maguire 2005) |
...
**Refinement — the trigger is transition-experience, not metric embedding (Garvert et al. 2017 [[wiki/papers/garvert-abstract-relational-map-2017.md]]):** Garvert's graph matches Kumaran & Maguire's social graph on the axes that predict *no* HC engagement — **discrete, non-spatial, no continuous organizing dimension** ("dimensions had to be extracted from the associations") — yet entorhinal/subicular adaptation reconstructs it. The distinguishing variable is *how the graph was acquired*: Garvert's was **sampled through experienced transitions** (random-walk sequences), whereas the social graph is known **declaratively without ever traversing it**. This sharpens the reconciliation: the EC map appears when the structure is learnable as a **transition operator** (and is encoded with an SR/communicability — weighted-future-state — metric, not a Euclidean one), not when it merely has metric geometry. Two caveats keep this from generalizing to all discrete reasoning: Garvert's graph was small and **2D-planar-embeddable** (MDS recovered it), and the design was **implicit/passive** — no map appeared outside HC (notably not vmPFC/OFC), the reverse of Constantinescu's vmPFC-dominant *active* abstract-grid task. See [[wiki/queries/mec-abstract-codes-vs-declarative-rules.md]] and [[wiki/queries/central-framing-epistemic-audit.md]].

**A *social* hierarchy does recruit HC/EC when embeddable + actively inferred (Park et al. 2021 [[wiki/papers/park-social-hierarchy-grid-code-2021.md]]):** the domain Kumaran & Maguire found gave *zero* MTL activation — a social relational structure — **does** produce an HC+EC 2D map (RSA ∝ 2D Euclidean distance) and an EC+mPFC hexagonal grid code once it is (i) **2D-metric-embeddable** (orderable competence/popularity ranks), (ii) reconstructed from **experienced binary comparisons** (transitive inference), and (iii) **inferentially traversed for a decision** (partner selection). This is the active/explicit counterpart to Garvert and confirms the operative variable is *embeddability + inferential use*, not social-vs-spatial content — a caution against reading K&M as "social ⇒ no hippocampus." Unlike Garvert's implicit/passive design, here the grid code extends *beyond* HC into mPFC/TPJ (perspective-taking) and is **coupled to decision-value coding** (value stronger for grid-aligned trajectories).

---

## Parallel Maps Along the HC Long Axis (Zheng et al. 2024)

Zheng, Hebart, … Garvert 2024 ([[wiki/papers/zheng-parallel-cognitive-maps-2024.md]]) reanalyze the Garvert 2017 dataset to ask how the hippocampal formation handles objects embedded in **two** relational structures at once: the newly-learned transition graph *and* their pre-existing **taxonomic-semantic similarity**. The answer is **parallel, anatomically separable maps — not one conjunctive map**:

| Structure | Code location | Acquisition | Metric |
|---|---|---|---|
| Transition (link distance) | Bilateral **entorhinal cortex**, more **anterior** | Recent, implicit, random-walk-sampled | SR / communicability |
| Taxonomic-semantic (category distance) | Bilateral **hippocampus**, more **posterior** | Lifetime, explicit, taxonomic | Category-similarity space (MDS 2D) |

- **Both are cognitive maps, both HC-formation-hosted, uncorrelated** (ρ≈0.03), each an independent fMRI-adaptation regressor; residual *perceptual* distance predicts nothing. The semantic map is **HC-specific** (absent from ATL/angular gyrus) and appears **passively** (semantics never task-demanded).
- **Non-conjunctive:** the two structures are not fused into an integrated map; each is null in the other's ROI. Contrast TEM's `p=f(g,x)`, which conjoins *structure×content* — here *structure×structure* stays separable.
- **Anterior–posterior gradient** along the long axis, echoing the coarse-anterior→fine-posterior **grid-scale gradient**, hinting at a shared organizational principle (statistical/associative structure anterior; taxonomic/semantic posterior).
- **Design implication:** the HC analog is a **bank of parallel maps** indexed by relational structure, not a single scaffold — a reasoning model over multi-relational entities should maintain separable maps and add a **map-selection (configurator) stage** (which structure is task-relevant now?) rather than force-integrating. This also refines the System-1 entry condition to **embeddability alone**: a lifetime-acquired, *explicit* taxonomic structure is mapped in HC, so declarative/explicit character is not the disqualifier — non-metric/non-embeddable structure is (see [[wiki/queries/mec-abstract-codes-vs-declarative-rules.md]]).

---

## Acquiring Arbitrary Cue→Action Mappings (Wise & Murray 2000)

Bilateral hippocampectomy (and fornix transection) severely impairs monkeys' acquisition of **novel** arbitrary visuomotor mappings while leaving **pre-lesion** mappings intact — the same monkeys still discriminate the cues and execute the movements, so the deficit is neither perceptual nor motor. This is the clearest behavioral instance of HC's time-limited role: HC is required to *bind* an arbitrary cue to an action, then drops out. Similar findings hold in humans; these lesions unintentionally reproduced **H.M.**'s amnesic profile in monkeys after decades of deliberate attempts had failed.

**Two caveats against over-reading it as a fast/slow circuit split:**
- **PMd lesions impair both novel and pre-lesion mappings.** The dissociation is *asymmetric* and HC-specific — premotor is the executor throughout, not a retention-only store that takes over from HC.
- **The neuroimaging and neuropsychology diverge.** Lesion work emphasizes *spatial* consequents (HC may serve visuospatial but not visuovisual mapping); human imaging finds hippocampal/parahippocampal activation for **nonmotor, nonspatial** associative learning (Henke et al.; novel word pairs, Dolan & Fletcher), implying a more general role. Unresolved — logged in [[wiki/empirical-tensions.md]].

**Weak, learning-selective activity (tentative sparse-coding signature).** Rolls and colleagues find HC learning-related activity changes during arbitrary mapping that are significant but weak — a few discharges/second, against robust changes in premotor cortex. Wise & Murray speculate this reflects a **sparsely coded** network vs. a densely coded one. HC changes are also predominantly *learning-selective* (activity falls as learning proceeds, often to silence) where premotor is *learning-dependent* (activity rises) — consistent with a time-limited memory role. The designs differ enough to preclude direct comparison, so this remains speculation. See [[wiki/concepts/arbitrary-mapping.md]] and [[wiki/concepts/sparse-distributed-representations.md]].

**What HS actually holds: three things, not one (Murray, Bussey & Wise 2000, [[wiki/papers/murray-bussey-wise-pfc-arbitrary-mapping-2000.md]]).** The companion review's CLS-consistent proposal is that HS is the **intermediate-term** store for *mappings, higher-order rules, and problem-solving strategies in parallel*, pending consolidation into frontal **cortical-BG modules** (recurrent cortex→BG→thalamus loops) as the long-term store. Two results sharpen this beyond the generic CLS picture:

- **HS lesions spare strategy.** Hippocampectomized monkeys keep applying the Repeat-Stay/Change-Shift strategy — which cuts initial 3-choice error 67%→33% — while failing to learn any new mapping. **PFv+o** lesions abolish the strategy instead. So although HS holds strategies in the intermediate term, once consolidated they live in PF modules and no longer need HS: the same time-limited profile as mappings, dissociated by lesion.
- **Two routes to spared familiar mappings.** Preserved pre-lesion performance could reflect either repeated cross-day exposure to the same mappings, *or* an intact long-term store. The HS data favour the second — familiar-mapping performance emerges *quickly* post-surgery without days of re-exposure — whereas preliminary data suggest the first interpretation may apply to PFv+o lesions instead. This is a within-paradigm dissociation of *why* two lesions both spare familiar mappings, and it is what makes HS specifically the intermediate-term store rather than a general contributor.

**Design consequence.** A fast store keyed only on `action → effect` implements one of the three things the biological fast store carries; the two it drops (rules, strategies) are precisely the ones that make binding fast. See [[wiki/queries/action-semantics-contextual-inference.md]].
...
## Navigation as a Three-Pillar Distributed Network (Epstein et al. 2017)

Epstein et al. 2017 ([[wiki/papers/epstein-cognitive-map-humans-2017.md]]) decompose cognitive-map navigation into **three separable pillars**, showing the HC/EC map is only *one* component of a broader network — a caution against treating "the host" of navigation (or of its abstract generalization) as a single locus:

| Pillar | Operation | Regions | Abstract analog |
|---|---|---|---|
| **Spatial coding** | Map/grid codes preserving distance | HC (place), EC (grid) | Grid code for social/conceptual spaces (Constantinescu, Park) — **demonstrated** |
| **Landmark anchoring** | Fix map orientation/position to stable references | RSC (anchor hub), PPA, OPA | **No demonstrated analog** — "no cells fire to the boundary of a concept" |
| **Route planning** | Retrieve path options + evaluate | HC (retrieves/traces trajectories), lateral+dmPFC (evaluates), PPC (allocentric→egocentric) | Prospective/episodic-future thought — speculative |

**Anchoring hierarchy:** boundaries and environmental geometry (spatially fixed) are stronger controllers of map orientation than objects (movable); after disorientation, animals reorient by chamber geometry even against featural cues. **RSC** mediates a **local↔global reference-frame transformation** — Jacob et al. 2017 *bidirectional cells* code heading in a local geometric frame while classical HD cells code global heading; RSC converts between them. This local-frame alignment is the biological "orientation" operation that the schema/configurator account needs but lacks (see [[wiki/concepts/memory-schemas.md]], context-retrieval vs. orientation).

**Topological vs. Euclidean coding — unresolved (Howard/Javadi):** during navigation of London's Soho, **EC tracks Euclidean distance-to-goal** while **HC tracks graph-topological quantities** — path distance plus *degree centrality* (# connected streets) and *closeness centrality* (global connectivity). The authors note it is "unclear how this topological coding of space relates to a possible Euclidean spatial code." The hippocampal formation therefore holds a **non-metric graph code even for physical space** — a middle term between the metric code and genuinely non-embeddable structure, and a candidate bridge to the latter. See [[wiki/empirical-tensions.md]] (topological-vs-Euclidean row) and [[wiki/queries/central-framing-epistemic-audit.md]].
...
- **[[wiki/concepts/temporal-context.md]]** — the Temporal Context Model's anatomical mapping is a precursor to the g/x/p split: items = cortical association areas, temporal context `t` = parahippocampal/EC, and the HC supplies α_N reinstatement of the prior EC context state; grounds the map-vs-memory dual role in a single leaky-integrator equation predating TEM.
...
- **[[wiki/concepts/arbitrary-mapping.md]]** — HC is required to acquire *novel* arbitrary cue→action mappings but not to express pre-lesion ones (the cleanest behavioral demonstration of its time-limited role, and the lesion that reproduced H.M.'s amnesia in monkeys); its weak, learning-*selective* activity changes during such learning are a tentative sparse-coding signature contrasting with premotor cortex's robust learning-*dependent* changes.
- **[[wiki/papers/wise-murray-arbitrary-mapping-2000.md]]** — source for the hippocampectomy novel-vs-pre-lesion mapping dissociation, the H.M. reproduction, the Rolls weak-activity-change result and its sparse-coding interpretation, and the lesion-vs-imaging divergence over whether HC's role here is spatial-specific or general.
- **[[wiki/papers/murray-bussey-wise-pfc-arbitrary-mapping-2000.md]]** — source for HS as the intermediate-term store holding mappings, higher-order rules, *and* strategies in parallel pending consolidation into frontal cortical-BG modules; the HS-spares-strategy / PFv+o-abolishes-strategy dissociation; and the two-routes argument that distinguishes an intact long-term store from mere cross-day re-exposure.
