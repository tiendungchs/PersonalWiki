...
|---|---|---|---|---|
| **Hypothesis-Testing (verbal)** | Rapid explicit WM-based rule search + BG for rule switching; verbalizable criteria tested and discarded sequentially | DLPFC WM maintenance + BG Go/NoGo for rule updates | Fast inner loop: rule changes within-episode | Fails on information-integration tasks where the optimal rule cannot be verbalized |
| **Procedural (implicit)** | Slow dopamine RL on striatal stimulus-response weights; gradual accumulation of input–output associations | Caudate nucleus (dorsal striatum) → slow DA RL (TD-like) | Slow outer loop: accumulates across many trials | Fails under long feedback delays (>2 s) that break the DA reinforcement signal |
...
- **Feedback delay dissociation:** Delay of 2.5–5 s between response and feedback selectively impairs the procedural (but not hypothesis-testing) system — the delayed DA signal cannot support corticostriatal RL (Maddox et al. 2003). This confirms the two systems are computationally independent.
- **DA depletion selectivity:** Parkinson's disease patients (caudate DA depletion) are selectively impaired on procedural categorization while hypothesis-testing performance is relatively spared (Knowlton et al. 1996) — a double dissociation with amnesia patients, who show the reverse pattern.
- **Meta-learning dual structure:** COVIS implements slow outer loop (procedural RL accumulates category weights across training) and fast inner loop (hypothesis testing updates rule within-episode) in the same BG circuitry — anticipating the Wang et al. 2018 meta-RL account by 20 years.
...
- Does not incorporate the three-pathway (Go/NoGo/Hold) account of BG; only the direct (Go) pathway is modeled for procedural RL.
...
|---|---|---|---|
| **Fast inner loop** | Explicit verbal rule search in WM | BG-gated PFC stripe activations | LSTM recurrent dynamics over full (o,a,r) history |
| **Slow outer loop** | Striatal DA RL for S-R weights | PVLV DA trains BG Go/NoGo stripe weights | A3C DA trains PFC+BG synaptic weights across episodes |
...
- **[[wiki/entities/basal-ganglia.md]]** — the procedural system uses the same dorsal striatum → DA RL circuit as the BG direct pathway; the three-pathway account (Go/NoGo/Hold) is the mechanistic upgrade over COVIS's direct-pathway-only model.
- **[[wiki/concepts/working-memory.md]]** — the hypothesis-testing system depends on explicit WM maintenance in DLPFC; BG gates which rule is held (Go) or discarded (NoGo) in the current WM slot.
- **[[wiki/concepts/cognitive-control.md]]** — the hypothesis-testing system implements set-shifting (the third CC component) as its primary fast operation; COVIS shows that the same BG circuitry that gates WM slots also switches between rule candidates.
