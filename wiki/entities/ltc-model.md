...
**Biological origin**: The LTC equation derives from the nonspiking neuron cable equation:
- `dv/dt = -gl·v(t) + S(t)` (leaky integration; Lapicque 1907)
- `S(t) = f(v, I)·(A-v)` (steady-state synaptic current; Koch & Segev 1998)
- Combined: identical to LTC Eq. 1, with `1/τ ↔ gl`. Source: C. elegans nematode circuit models (Hasani et al. 2020; Gleeson et al. 2018).

---

## Key Results

| Task | LTC | Best baseline | Note |
|---|---|---|---|
...
| **CfC** (Hasani et al. 2022, NMI) | Analytical closed-form ODE approximation; time-continuous gating | **None** (solver-free) | 1–5 orders of magnitude speedup; matches or exceeds LTC across all benchmarks |
...
CfC and Liquid-S4 eliminate the ODE solver entirely — CfC via analytical approximation, Liquid-S4 via SSM causal convolution — making LTC principles scalable without stiff integration.
...
- **No variable-delay mechanism**: unlike TRNN's neuron recruitment, LTC has no tested mechanism for multi-second WM maintenance
...
| **TRNN** | Discrete-step self-inhibition | Not analytically proven | Medium (dPCA manifold) | Yes (variable delay) | C. elegans + mammalian PFC |
...
- **[[wiki/concepts/neuromodulation.md]]** — the liquid τ is a local mechanistic analog of neuromodulatory time-constant modulation: just as ACh, DA, and NA shift neural integration timescales globally, τ_sys adapts per-neuron integration windows based on current inputs continuously.
- **[[wiki/concepts/predictive-coding.md]]** — the LTC ODE structurally resembles Friston's bilinear Dynamic Causal Models (DCMs), which are used to model fMRI time-series and have inherent causal structure; LTCs may serve as causal temporal generative models in a FEP framework.
...
- **[[wiki/entities/trnn-model.md]]** — both achieve expressive temporal dynamics but via different mechanisms: TRNN via self-inhibition + sparse + hierarchical topology (discrete-step); LTC via liquid τ (continuous-time ODE); TRNN is better suited for multi-item WM and long delays; LTC for continuous-time signal modeling with stability guarantees.
- **[[wiki/concepts/working-memory.md]]** — LTC is a continuous-time WM candidate for short-to-medium timescales; the liquid τ allows each neuron to self-select its memory timescale based on input statistics, but vanishing gradients limit long-range applicability.
- **[[wiki/concepts/attention.md]]** — Liquid-S4 fuses LTC liquid kernels into the SSM DPLR framework, bridging continuous-time adaptive-memory dynamics to the attention/transformer thread; the liquid kernel encodes multi-way input correlations as an additive term on top of the SSM recurrence, achieving 87.32% LRA SOTA.
