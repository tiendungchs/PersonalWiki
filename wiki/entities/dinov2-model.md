...
| DINO head | MLP → softmax prototype distribution (image-level CLS loss) |
...
- EMA teacher-student requires aligned architectures; cross-modal or asymmetric-branch training is not natively supported (VICReg/branch-independence addresses this).
...
| Collapse prevention | EMA + KoLeo | EMA + KoLeo + Gram | EMA (SIGReg in LeJEPA) |
...
- **[[wiki/entities/jepa-model.md]]** — DINOv2's EMA teacher-student design (same momentum schedule 0.994→1.0), iBOT masked patch prediction, and KoLeo regularizer are all directly inherited by I-JEPA / V-JEPA 2; DINOv2 is the architectural ancestor of the JEPA family.
