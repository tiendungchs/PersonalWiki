...
## Program Induction / Abduction: The Rival Reduction (Johnson et al. 2021)

The first behavioral study of humans on ARC ([[wiki/papers/johnson-human-program-induction-arc-2021.md]]) frames abstract reasoning as **program induction** and, critically, argues ARC demands **abduction** (generate a new rule on the fly) rather than **induction** (search a fixed primitive set) — the primary *non-navigational* rival to the latent-graph-discovery framing on the wiki's own target benchmark. Humans **83.8%** vs. Kaggle program-synthesis winner 21% (test) / 57.5% (these 40 tasks).

| Standard (p)LoT assumption | What ARC behavior shows | Consequence for model design |
|---|---|---|
| Fixed hypothesis space, primitives defined in advance | ARC tasks hand-designed *without* a grammar → solvers bind new variables / invent rules per task | The vocabulary is **not pre-given** (Gap #3); a reasoner must abduce primitives, not select them |
| Symbolic parse of the stimulus is fixed | What counts as an "object" is *task-dependent* (occlusion, grouping); humans re-parse per task | Objectness must be a **flexible, context-gated** parse, not a fixed symbolic tokenization ([[wiki/concepts/core-knowledge.md]]) |
| Hypotheses generated from a minimal symbolic LoT | Largest unique-word classes are geometric + transformation; description length **negatively predicts accuracy** (b=−0.17, r=−0.50); within-task naming divergence 0.41 ≪ shuffled 0.68 | **Natural language scaffolds hypothesis generation** — a compositional substrate that is neither a metric map nor a minimal primitive set |

**Object-centric planning as behavioral objectness.** State-space graphs of participants' actions show bottleneck states = task-relevant *objects*; humans build outputs object-by-object or copy-then-move objects. Human errors **obey object priors** (correct shapes/colors, one alignment axis); Kaggle errors **violate** them (elongated shapes, grid wrap-around) — a precursor to the ConceptARC near-miss / concept-failure dissociation and Beger 2025's missing-objectness finding ([[wiki/concepts/shortcut-reasoning.md]]).

**Epistemic status (central-framing audit).** This is Axis-2 evidence that a rival reduction explains ARC *without* navigation, on the discrete-symbolic slice where "reasoning ≅ navigation" is a bet. It does not falsify the navigation frame (it is an algorithmic account with no neural claim, and program search is *foldable* into hypothesis-space navigation), but it demotes the strong claim toward "one contestable frame among several." See [[wiki/queries/central-framing-epistemic-audit.md]].

**The framework behind the rival: PLoT (Goodman, Gerstenberg & Tenenbaum 2024).** Johnson supplies the *behavioral* case; the **Probabilistic Language of Thought** ([[wiki/concepts/probabilistic-language-of-thought.md]]) is the formal framework it instantiates, and it formalizes all three required ingredients above natively: **compositionality** = function composition of stochastic programs; **causality** = a generative program + counterfactual-by-intervention on its execution trace; **learning-to-learn** = program induction via a program-generating meta-program whose library evolves (concepts become primitives → changing inductive bias). It reframes the audit's contest at the framework level: PLoT claims the same "one problem" universality as LGD but with a *universality theorem* rather than a framing bet, and — being computational-level — the two may be complementary across Marr levels rather than rivals.
...
- **[[wiki/papers/johnson-human-program-induction-arc-2021.md]]** — the program-induction / abduction rival frame on ARC: humans generate rules on the fly from an unbounded conceptual store (not a fixed primitive set), re-parse objects per task, and use language to scaffold hypotheses; the primary non-navigational reduction and the Axis-2 anchor of the central-framing audit.
- **[[wiki/concepts/probabilistic-language-of-thought.md]]** — the formal framework (Goodman et al. 2024) behind the program-induction rival: it formalizes compositionality/causality/learning-to-learn as function composition / generative-program-with-intervention / library-learning, and is the computational-level "one problem" competitor to LGD with a universality theorem.
- **[[wiki/queries/central-framing-epistemic-audit.md]]** — grades "reasoning ≅ navigation over a latent graph" into three sub-claims; the program-induction section above is the Axis-2 rival that demotes the strong reduction toward "one contestable frame" on the discrete-symbolic slice.
