...
- **[[wiki/queries/action-semantics-contextual-inference.md]]** — COIN's nonparametric (Dirichlet-process) context prior is the formal, implemented version of the accommodation trigger this page describes qualitatively: a new context/schema is instantiated exactly when no existing one explains the observations, making "assimilate vs. accommodate" a computable posterior test rather than a heuristic.
