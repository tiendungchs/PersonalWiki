...
**What the γ parameter must produce — the common-currency subjective-value signal (Kable & Glimcher 2007, [[wiki/papers/kable-glimcher-subjective-value-2007.md]]):** Doya's 5-HT=γ is a *metaparameter*; Kable & Glimcher supply the *output variable* it controls. Human fMRI shows **ventral striatum, vmPFC, and posterior cingulate** explicitly encode the behaviorally-revealed subjective value `SV = A/(1+kD)` (hyperbolic; k = subject-specific discount rate, spanning 200× across people). The **neural discount rate equals each subject's behavioral rate** (psychometric-neurometric match) — the idiosyncratic preference function is physically instantiated, not a group constant. Two design consequences: (a) the discount factor is a *per-agent latent* read out of a single value node, not a fixed hyperparameter; (b) **a single common-currency value scale covers rewards at all delays** — this *falsifies* the β–δ two-system view (McClure 2004) that a limbic-impulsive system values immediate and a cortical-patient system values delayed rewards. Relevant to the wiki's dual-system commitments: valuation is *one* signal even where control (MB/MF, System 1/2) is dual. Maps to Block 3D's `V(g)` head ([[wiki/queries/proposed-reasoning-model-block-architectures.md]]).
...
## Dorso-Ventral mPFC Neuromodulatory Gradient (Heidbreder & Groenewegen 2003)

Neuromodulator delivery is **not uniform across the mPFC**: it follows the dorso-ventral functional axis ([[wiki/entities/prefrontal-cortex.md]], [[wiki/papers/heidbreder-groenewegen-mpfc-dorsoventral-2003.md]]). This matters for the reasoning model because the **ventral (PL/IL) mPFC is the System-2 rule-switching/schema-selection substrate** ([[wiki/queries/proposed-reasoning-model-block-architectures.md]]) — so its neuromodulatory profile *is* the profile of the neuromod bus feeding System 2.

| Modulator | Dorsal (ACd/PLd/FR2) | Ventral (PLv/IL/MO) | Metaparameter reading |
|---|---|---|---|
| **DA** | SNc/A9 source, superficial layers, **high DAT** → fast uptake → low extracellular DA | VTA/A10 source, deep layers, **low DAT** → high extracellular DA / steep concentration gradient | Ventral (System 2) runs in a **higher-DA regime**; the D1 inverted-U WM-stability dial (Block 3B) is subregion-tuned, not global |
| **ACh** | infusion does *not* affect spatial WM | scopolamine into PL/IL **impairs** spatial WM (rescued by oxotremorine) | ACh = α write/WM support is **localized to the ventral System-2 substrate**, not the dorsal sequencer |
| **5-HT** | — | IL ↔ dorsal raphe (reciprocal); prelimbic 5-HT outflow ↑ → impulsivity | 5-HT = γ (discount/horizon) enters via the ventral pole; low horizon ↔ impulsive premature responding |
| **NA** | ACC lowest NA density in neocortex | LC ↔ PL/IL reciprocal; NA can **increase DA release** in PL/IL | NA (β/explore) and DA are coupled specifically in the ventral substrate |

**Design implication:** the neuromod bus is not one global broadcast but is **read out differently by the two mPFC subsystems**. The ventral (System-2) substrate is the high-DA, ACh-WM-gated, 5-HT-horizon-set, brainstem-feedback-heavy pole — consistent with the bilaminar DA source split (superficial A9 vs deep A10) providing *separately addressable* DA channels to the sequencer vs the rule-switcher. A model that applies one scalar DA/ACh gate uniformly across its PFC analog cannot reproduce the dorsal-sequencing / ventral-rule-switching neuromodulatory dissociation.
...
- **[[wiki/papers/kable-glimcher-subjective-value-2007.md]]** — supplies the output variable of the 5-HT=γ metaparameter: a common-currency subjective-value signal in vmPFC/ventral striatum/PCC whose neural discount rate matches each subject's behavioral rate; falsifies the β–δ two-system valuation account (one value scale for all delays).
...
- **[[wiki/papers/heidbreder-groenewegen-mpfc-dorsoventral-2003.md]]** — source for the dorso-ventral mPFC neuromodulatory gradient: ventral PL/IL (the System-2 rule-switching substrate) is the high-extracellular-DA (VTA/A10, low-DAT), ACh-WM-gated, 5-HT-horizon pole vs the dorsal low-DA (SNc/A9, high-DAT) sequencer; grounds subregion-specific (not global) neuromodulatory read-out of the neuromod bus.
- **[[wiki/queries/proposed-reasoning-model-block-architectures.md]]** — the proposed reasoning model's neuromod bus feeds a ventral-mPFC-analog System 2; the dorso-ventral gradient specifies why that bus must be read out per-subsystem rather than as one global scalar gate.
