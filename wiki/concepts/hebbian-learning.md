...
| **BTSP** | Seconds | EC3-instructed plateau potential (NOT postsynaptic firing) opens a ~10s plasticity window; binary weights updated with stochastic ±1 rule depending on current weight value and timing relative to plateau onset; stochastic gating parameter $f_q$ ≈ 0.005 (probability of plateau per pattern) distributes memory load uniformly across CA1 neurons; creates **bimodal weighted-sum distribution** (plateau-neurons cluster high, others cluster low) enabling recall from up to 33% masked cues; LTD branch **actively repels** memory traces for similar patterns. Wu & Maass 2025 [[wiki/papers/wu-maass-btsp-2025.md]]: binary-weight BTSP matches continuous-weight Hopfield at $f_p$ = 0.005 sparsity; analytically tractable. See [[wiki/concepts/btsp.md]] for full treatment. | Single-shot node acquisition in CA1; fast-M write with binary weights; memory load balancing; repulsion effect for similar patterns |
...
- **eHebb (error-Hebbian)** (Shervani-Tabar & Rosenbaum 2023): $\Delta W_{\ell-1,\ell} = -\mathbf{e}_\ell\mathbf{e}_{\ell-1}^T$ — replaces activation co-activity with error co-activity; for a linear network, $\mathbb{E}[\mathbf{e}_\ell\mathbf{e}_{\ell-1}^T|B]\propto B^T$, pushing W toward the transpose of the random feedback matrix B and aligning forward weights with fixed random feedback connections; creates an auxiliary information channel that accelerates backward alignment within the same learning iteration (vs. two-iteration delay for pure FA). Discovered by meta-learning over 10 candidate local plasticity terms; biological candidate: co-burst events of pyramidal neurons at adjacent cortical layers encode the error co-activity product via burst/spike multiplexing.
...
| **HRN symbolic graph (Zhao et al. 2022)** | **Hebb rule on SNN concept nodes** | **One-shot write of perceived scene content: if ANN detects "object A is red," Hebb rule connects node "red" ↔ node "object A" with positive weight — no backpropagation, no explicit supervision** |

---

## For Building a Reasoning Model

- **Fast-M write:** the one-shot Hopfield rule is the computational fast-M write; BTSP is its biological implementation with binary weights and stochastic plateau gating. Any fast episodic binding layer needs a Hebbian-equivalent local rule — no backprop required. See [[wiki/concepts/btsp.md]] for the binary weight formalism, capacity analysis, and ML design implications.
...
- Stability of layered Hebbian learning (forward-forward, predictive coding): whether Oja's rule / BCM extend to multi-layer Hebbian networks without eigenvalue explosion is an open engineering question. Shervani-Tabar & Rosenbaum 2023 show that combining Oja's rule with eHebb in a meta-learned plasticity rule stabilizes multi-layer online training in practice, but theoretical stability bounds remain uncharacterized.
...
- **[[wiki/entities/hnn-framework.md]]** — HRN's one-shot Hebb binding (ANN-detected object attributes → spike stimuli → Hebb-written SNN concept edges) is the clearest published demonstration of fast-M Hebb writes for symbolic content rather than spatial/episodic content; the designable HU converting attribute detections to spike trains is the interface between ANN continuous grounding and Hebb SNN binding.
- **[[wiki/concepts/btsp.md]]** — the dedicated concept page for BTSP: formalizes the binary stochastic rule, provides the analytical capacity theory ($p_e$/$p_o$ expressions), establishes the bimodal distribution mechanism for recall robustness, and derives the repulsion effect from the LTD branch — all absent from the summary-level treatment here.
- **[[wiki/papers/wu-maass-btsp-2025.md]]** — primary source for the binary BTSP model; demonstrates that binary weights achieve comparable CAM quality to continuous Hopfield for sparse inputs ($f_p$ = 0.005); provides first analytical theory for BTSP memory systems and first learning-based model of the human repulsion effect.
- **[[wiki/papers/meta-learning-plasticity-random-feedback.md]]** — source for the eHebb rule and Oja's role in deep network training: meta-learning over local plasticity terms discovers that eHebb (error co-activity product) and Oja (unsupervised orthonormalization) together close the feedback alignment gap; provides the mathematical proof that eHebb pushes W toward B^T in expectation, grounding it as an error-driven Hebbian variant with a clear mechanistic interpretation.
