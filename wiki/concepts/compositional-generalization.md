...
**The lexical/structural split — the remaining hard boundary.** On SCAN lexical splits (add jump, around right, opposite right) MLC achieves ≤0.22% error; on COGS lexical types 0.87% — basic seq2seq is ≥7× worse on both. But on SCAN length (productivity) and COGS structural types, MLC scores **100% error** — same as random. Meta-learning generalizes when the test episode is in-distribution with respect to training grammar *variation* (new vocabulary with known structure); it fails when episode *types* themselves are out-of-distribution (new sentence structures, sequences longer than training). The LRM knowledge-boundedness ceiling applies here: the fast inner loop is bounded by the slow outer loop's training distribution.

---

## PGM: Recombination vs. Decomposition in the Visual Domain

Barrett et al. 2018 ([[wiki/papers/pgm-barrett-2018.md]]) provide visual-relational evidence for the same composition-decomposition asymmetry that Hupkes et al. probe in sequence-to-sequence tasks. WReN (a pairwise Relation Network) on Raven-style PGM matrices shows:
