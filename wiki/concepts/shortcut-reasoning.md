...
**Externally-supplied objectness (Das et al. 2025, [[wiki/papers/das-compositional-neurosymbolic-arc-2025.md]]):** the CoreThink ARC-AGI-2 reasoner sidesteps the *learning* problem by **hand-coding** the objectness parse — deterministic 8-connected-component decomposition + per-object bounding box, centroid, canonical shape, color histogram, and cavity detection — before any LLM rule induction. Injecting this object-level abstraction is the single largest performance driver in their ablation (+6.9pp on ARC-AGI-2, vs. +3.9pp from self-consistency sampling), directly quantifying how much of the ARC shortcut gap is *just* the missing objectness parse. Caveat: this is objectness *supplied*, not *acquired* — it confirms the prior's value while leaving open-problem #1 (learning objectness without hand-coding) untouched, and its fixed connected-component parse inherits the same 4/8-connectivity bias listed above whenever the true object grouping is non-connectivity-based.
...
- **[[wiki/papers/das-compositional-neurosymbolic-arc-2025.md]]** — quantifies the missing-objectness shortcut from the *fix* side: hand-coding the connected-component object parse (objectness supplied externally) is the +6.9pp dominant driver on ARC-AGI-2, showing how much of the gap is the absent discrete-entity parse rather than the rule-induction step — while leaving objectness *acquisition* unsolved.
