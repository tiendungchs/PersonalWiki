...
- **[[wiki/queries/reasoning-as-coupled-navigation-strategizing.md]]** — casts HC/DMN replay as the **generator** in the Strategist's refinement loop: sampled candidate sequences (asymmetric-STDP chaining) are rolled forward through the Navigator-as-simulator and scored against `G_goal` by vmPFC/ACC (the verifier). Goal-directed *seeded* replay = preplay biased by mPFC; *spontaneous* firing = the exploratory prior that stops the search collapsing onto known paths — and offline replay is the mechanism that *compiles* an effortful searched path into fast System-1 intuition.
