...
- **[[wiki/concepts/arbitrary-mapping.md]]** — supplies a *negative* constraint on this page's scope: the profound PFv+o arbitrary-mapping deficit occurs with the instruction stimulus on-screen through choice and execution (near-zero storage demand), and PFv-lesioned monkeys hold visual stimuli for 8s in delayed matching (Rushworth 1997). Murray, Bussey & Wise read this as falsifying "the widely promoted hypothesis that the sole specialized function of PF, as a whole, is the mediation of working memory" — so PFC delay activity is not prima facie evidence of a buffer, and a large PF function (binding arbitrary cue→action mappings) sits entirely outside this page's subject.
- **[[wiki/concepts/contextual-inference.md]]** — assigns WM an unusually specific *content*: the currently estimated **context responsibilities** — "which model am I using" — rather than task stimuli or goals. The evidence is a causal dissociation: a WM task inserted after counter-exposure abolishes spontaneous recovery and produces evoked-recovery-like behaviour, modelled as selectively wiping the memory of the last trial's responsibilities while leaving the learned transition probabilities (and every stored state) intact. A candidate functional spec for the delay-period activity this page characterizes mechanistically.
- **[[wiki/concepts/temporal-context.md]]** — TCM's drifting context is a leaky-integrator form of short-term memory that produces the recency effect without a discrete buffer; EC-V graded persistent firing (Egorov 2002) is its cellular substrate — a concrete data point in the persistent-vs-transient WM debate (integrator-cell persistence, activity-dependent divisive gain for normalization).
