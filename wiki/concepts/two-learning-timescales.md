...
- **[[wiki/queries/action-semantics-contextual-inference.md]]** — applies the split to *action semantics*: the arbitrary-visuomotor-mapping literature dissociates the circuits exactly along it (HC + ventral PFC acquire novel arbitrary mappings; premotor–striatal circuits retain overlearned ones), so slow W should learn the *manifold of possible operators* and fast M should one-shot bind action-pointer → operator per environment — never a trained `action → effect` function, which averages conflicting contexts.
