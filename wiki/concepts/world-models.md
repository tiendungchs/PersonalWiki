...
related: [wiki/entities/jepa-model.md, wiki/concepts/energy-based-models.md, wiki/concepts/predictive-coding.md, wiki/concepts/hierarchical-representations.md, wiki/concepts/abstract-reasoning.md, wiki/entities/tem-model.md, wiki/entities/ltc-model.md, wiki/entities/dnc-model.md, wiki/papers/lecun-path-towards-autonomous-intelligence-2022.md, wiki/papers/vla-survey-kawaharazuka-2025.md, wiki/papers/v-jepa-2-assran-2026.md, wiki/papers/leworldmodel-maes-2026.md, wiki/papers/lecun-jepa-critical-review-lett-2025.md, wiki/entities/iwmt.md, wiki/papers/safron-iwmt-expanded-2022.md, wiki/concepts/continual-learning.md, wiki/papers/kessler-continual-dreamer-2023.md, wiki/papers/embodied-intelligence-survey-2025.md, wiki/entities/spacetime-attractor.md, wiki/concepts/planning-as-inference.md, wiki/papers/mechanistic-planning-pfc-jensen-2026.md, wiki/papers/emergent-linear-world-models-nanda-2023.md, wiki/concepts/latent-graph-discovery.md, wiki/queries/central-framing-epistemic-audit.md, wiki/concepts/probabilistic-language-of-thought.md, wiki/papers/linear-spatial-world-models-tehenan-2025.md, wiki/papers/structured-maze-transformers-2023.md]
---
...
| **OthelloGPT (Li 2023; Nanda 2023)** | Board-state graph emergent from next-move SSL | Discrete move tokens | Softmax over legal moves | Single game; *spontaneous* world model, linearly encoded relative to current player, causally steerable |
| **LLM spatial WM (Tehenan et al. 2025)** | Linear R³ object-position subspace emergent from *text-only* pretraining; relations compose as vector sums | Discrete language tokens | Softmax over vocabulary | General-purpose LLaMA-3.2; *spontaneous* metric world model under *partial* observability, linearly encoded (post-PCA), causally steerable (74.3%, asymmetric); static only — transition function untested |
| **Maze-solving GPT (Ivanitskiy et al. 2023)** | Maze *connectivity graph* emergent from next-token path-prediction; adjacency heads respect topology in-context | Discrete coordinate tokens | Softmax over next coordinate | *Spontaneous* relational-graph world model, linearly decodable from a *single* token at layer 2; correlational (no ablations); emergence tracks grokking-like generalization jumps; forms only in the larger `jirpy` model (`hallway` solves its task without it — present ≠ necessary) |
...
**Nuance — "shallow" is a matter of *degree*, not a categorical absence (Tehenan et al. 2025):** a text-only LLaMA-3.2-3B nonetheless builds a *linear, causally-used* spatial world model — a subspace ≈ isomorphic to R³ where opposite relations are antipodal, independent relations orthogonal, and composed relations equal the vector sum of atomic ones (`above + left ≈ above-left`); activation-steering the subspace changes the model's stated relations (74.3%). This sharpens Nanda's observability point: OthelloGPT's board is *fully* observed by moves, whereas language only *partially* observes physical space — yet a coherent metric model still emerges, so the model is capturing structure *behind* the text, not merely text statistics. What stays genuinely shallow is *dynamics*: only the static configuration is present; the transition function (movement, physics, object permanence) is untested and likely where grounding is actually missing.
...
- **[[wiki/papers/emergent-linear-world-models-nanda-2023.md]]** — OthelloGPT is the cleanest existence proof that a *grounded* world model (the board = a relational state graph) emerges spontaneously from pure next-token SSL, is **linearly** encoded (relative to the current player), and is **causally used** (vector-arithmetic steering changes predictions) — a sharper contrast to the "LLMs are shallow world models" claim: the difference is *observability* (Othello moves fully determine the world; language only partially observes it), not next-token training per se. Its MoveFirst multiple-circuits result also shows a world model can be present-and-causal yet non-exclusive (shortcut circuits coexist).
- **[[wiki/papers/linear-spatial-world-models-tehenan-2025.md]]** — the text-only-LLM sibling of Nanda: a general-purpose LLaMA-3.2 spontaneously builds a *linear R³ spatial world model* (relations compose by vector addition), causally steerable (74.3%) — extends the emergence result from a fully-observable game to *partial-observability language*, directly nuancing the "LLMs are shallow world models" row above. Adds a "present ≠ uniformly steerable" caveat (steering asymmetry: `behind` 5% vs. `above` 100%) and shows the world model is *static only* — the transition function is untested, isolating dynamics as the genuine shallowness.
- **[[wiki/papers/structured-maze-transformers-2023.md]]** — the third Axis-4 emergence datapoint and the most literally *relational-graph* one: a maze-solving GPT compresses the entire maze *connectivity graph* into a single token (linearly decodable at layer 2), grows adjacency heads that respect topology in-context, and forms these representations *in lockstep with grokking-like generalization jumps* — the one emergence source that links world-model formation to a training-dynamics generalization signature. Two hedges its siblings sharpen: evidence is *correlational only* (no causal steering), and the map forms only in the larger model (`hallway` solves its task without it → present ≠ necessary, the training-side mirror of Nanda's MoveFirst).
- **[[wiki/concepts/probabilistic-language-of-thought.md]]** — a Church program *is* a generative world model: sampling semantics = mental simulation (noisy-Newtonian intuitive physics), and counterfactuals are computed by *intervening on the execution trace* (Pearl 3-step); the symbolic-generative cousin of JEPA/RSSM, predicting distributions over structured possible worlds rather than pixel or representation-space frames.
