...
- **[[wiki/queries/operator-collapse-in-fused-structural-codes.md]]** — a worked case of the input DG exists to handle: consecutive world-model codes at cos 0.994 despite genuinely different frames (~148 of 4096 cells changed). "Separate before you store" applies upstream of an *operator* as well as upstream of a memory — an expansion→sparse-competition stage on the encoder output is one of the ranked fixes for operator collapse.
