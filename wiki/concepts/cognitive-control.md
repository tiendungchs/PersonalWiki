...
## Creating, Clustering & Generalizing Task-Sets (Collins & Frank 2013)

Badre 2010 shows the hierarchy *searches in parallel*; Collins & Frank 2013 ([[wiki/papers/collins-frank-cts-task-set-2013.md]]) supply the complementary operation — how the higher level **grows and reuses** its own rule vocabulary rather than searching a fixed one. Their **C-TS (context–task-set) model** is a non-parametric Bayesian (Dirichlet / Chinese-restaurant process) clustering of contexts onto latent task-sets:

- A **task-set** = a set of stimulus→action→outcome contingencies; a **context** cues a TS but not 1:1 (many contexts → one TS).
- On a novel context: **reuse** an existing TS with prob ∝ its popularity, or **create** a new one with prob ∝ concentration **α**. Low α = aggressive clustering/generalization; high α = flat (one state per context).
- Tractable online inference: collapse the posterior onto a single MAP task-set each trial (single-particle filter) — only that TS's weights update.

**The load-bearing empirical finding — incidental over-structuring.** Humans spontaneously impose task-set structure on a *flat* problem that neither requires nor benefits from it, and pay for it: **positive transfer** when a new context maps to a known TS, **negative transfer** (elevated neglect-context errors) when a new context overlaps old TSs. The bias to build hidden structure is default-on, even against short-term optimality — the cognitive-control-side twin of the eager-γ / below-chance-no-rule result ([[wiki/papers/boettiger-desposito-sr-rule-learning-2005.md]]) and COIN's aggressive splitting ([[wiki/concepts/contextual-inference.md]]).

**Design implication (Block 3B/3C):** the rule hierarchy needs not just parallel search over a fixed rule bank (Badre) but a **create-or-reuse operator** — a concentration-parameter-gated decision to instantiate a fresh latent rule vs. cluster the new situation under an existing one. α is a first-class design dial trading generalization (reuse) against interference-free new learning (create). Crucially this grows **representational** task-sets (context clusters), not **temporal** options/subgoals — it does not by itself supply HRL goal decomposition. See [[wiki/queries/hrl-goal-decomposition-coverage.md]].
...
## Inhibition as the Substrate for Latent-Cause Reasoning (Sherwood et al. 2008)

Sherwood et al. ([[wiki/papers/sherwood-natural-history-mind-2008.md]]) elevate **inhibition** — the "common CC factor" (Miyake bifactor result above) — from a motor-suppression function to the enabling primitive for abstract, causal reasoning:

> Reasoning about unobservable causes requires **inhibiting stereotyped responses to superficially similar stimuli**, treating them differently when the inferred hidden cause differs. "Cognitive self-control" = suppressing stimulus-capture attention so that latent-cause inference, not surface similarity, drives the response.

This reframes the surface-feature-shortcut vs. model-building distinction ([[wiki/concepts/abstract-reasoning.md]], [[wiki/concepts/shortcut-reasoning.md]]) in cognitive-control terms: **shortcut reasoning is a failure to inhibit prepotent surface-feature responses**; abstract reasoning is what the goal-maintained inhibitory bias buys once it out-competes the stimulus-driven response. Comparative evidence: greater ape/human inhibitory control (delayed gratification, tolerance) — an executive function attributed to enlarged DLPFC — is proposed as the phylogenetic precursor to flexible, latent-cause-sensitive behavior.

**Design implication:** the biased-competition equation `P(response | state, goal) ∝ P(response | state) × exp(β·b(g*, response))` is exactly the mechanism that lets an inferred-cause goal `g*` override a surface-similarity prior `P(response | state)`. A reasoning model without a strong, maintainable inhibitory bias will default to the prepotent surface response — the mechanistic root of shortcut learning. Inhibition strength (β, goal-maintenance persistence) is therefore a first-class design parameter for abstraction, not just for stop-signal tasks.
...
- **[[wiki/papers/sherwood-natural-history-mind-2008.md]]** — source for inhibition-as-latent-cause-substrate: "cognitive self-control" (suppressing surface-feature capture so hidden-cause inference drives the response) reframes shortcut vs. model-building reasoning in cognitive-control terms; enlarged DLPFC inhibitory control as the phylogenetic precursor.
- **[[wiki/concepts/abstract-reasoning.md]]** — the observable/unobservable-cause decoupling is enabled by inhibitory control; surface-feature capture is the failure to inhibit, latent-cause inference is what a maintained inhibitory bias buys.
...
- **[[wiki/concepts/arbitrary-mapping.md]]** — binding an arbitrary cue→action alphabet is a CC problem in miniature: SMA/pre-SMA holds the bank of contextually appropriate *candidate* mappings and dlPFC biases which candidate wins, so goal-maintenance-as-top-down-bias (Miller & Cohen) is what installs a new action vocabulary, not merely what applies a known one.
- **[[wiki/papers/boettiger-desposito-sr-rule-learning-2005.md]]** — human fMRI complement to the tactic cascade above, in the acquisition regime: the same medial premotor complex that Awan/Matsuzaka show buffering *known* tactics is here the candidate bank for *unknown* mappings, biased by dlPFC; dlPFC–SMA coupling explains 63% of variance in learning success while neither region's activity does — the coupling, not the module, is the bottleneck. Note the ordering caveat: dlPFC engages only when the rule is *categorical*, so this is the same rostro-caudal hierarchy Badre 2010 formalizes, five years earlier and without the RL framing.
- **[[wiki/papers/collins-frank-cts-task-set-2013.md]]** — source for the create/cluster/generalize task-set operation: Dirichlet-process clustering of contexts onto latent task-sets (reuse ∝ popularity vs. create ∝ α) implemented in nested corticostriatal loops; the incidental-over-structuring result (positive + negative transfer on a flat problem) is the CC-side evidence for a default-on bias to build hidden structure.
- **[[wiki/concepts/contextual-inference.md]]** — C-TS is the cognitive-control ancestor of COIN: the create-or-reuse task-set gate is the same non-parametric latent-state growth COIN formalizes as HDP context inference; both make aggressive splitting the default, which cognitive control pays for as negative transfer.
- **[[wiki/concepts/hierarchical-reinforcement-learning.md]]** — the rostro-caudal PFC gradient here is the *representational* (rule-depth) hierarchy; HRL/options is its *temporal* (action-chunk) twin, and Botvinick maps both onto the same "higher = more anterior" axis — so Block 3C needs a temporal-abstraction operator (options + subgoal generator), not only nested rules.
