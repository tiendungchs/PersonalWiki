...
| **JEPA (LeCun 2022)** | x-encoder s_x and latent variable z | y-encoder s_y (abstract representation); encoder invariance discards unpredictable details | Minimize D(s_y, ŝ_y) in representation space + maximize information in s_x, s_y + minimize information in z; non-generative analog of PC's prediction objective — operates without explicit error neuron populations. |
...
| **Unified sub-symbolic cognition (Butz 2016)** | Active predictive encodings settle toward mutual agreement (global-posterior encoding = distributed local FE-minimization) | Distributed generative model over three primitive encoding types (temporal / spatial / top-down) shaped by structural priors; event-schema + episode encodings on top | Concept = an approximately consistent **free-energy-minimum attractor**; composition = merged attractor; episode = attractor succession — the FEP recast as a full **reduction of symbol-like thought**; the implementation-level rival to LGD/navigation and PLoT (see [[wiki/concepts/latent-graph-discovery.md]], [[wiki/concepts/probabilistic-language-of-thought.md]]) |
...
- **[[wiki/papers/butz-unified-subsymbolic-cognition-2016.md]]** — recasts the FEP as a *full reduction of cognition*: concepts = distributed free-energy-minimum attractors, compositions = merged attractors, episodes = attractor successions, built on three primitive encoding types (temporal/spatial/top-down) shaped by structural priors; the implementation-level "one-problem" rival to LGD and PLoT.
- **[[wiki/concepts/latent-graph-discovery.md]]** — the navigation reduction; Butz's free-energy account is the *third* mutually-foldable "one-problem" frame beside LGD and PLoT — navigation-attractor ⊂ free-energy-attractor, while spatial coding is only 1 of Butz's 3 primitives, so the FE frame de-privileges navigation even as *substrate* (central-framing audit, Axis-2 #7).
- **[[wiki/concepts/probabilistic-language-of-thought.md]]** — the program-induction reduction; PLoT and the free-energy account both claim "one problem" but at different Marr levels (PLoT computational/silent-on-implementation; Butz implementation/attractor-dynamics), making them cross-level complements rather than same-level rivals — the three-frame stack the central-framing audit resolves C into.
- **[[wiki/queries/central-framing-epistemic-audit.md]]** — Butz is Axis-2 #7, the source that closes the audit: the free-energy attractor account de-privileges navigation as *substrate* (not just as reduction), completing the three-frame demotion of sub-claim C to a chosen lens.
- **[[wiki/queries/operator-collapse-in-fused-structural-codes.md]]** — the linear subtraction `ε = x − g(x̃)` is what keeps a static background out of the learning signal entirely: a perfectly-predicted common mode contributes zero to the ascending error. A world model *without* the subtraction lets that common mode dominate the code norm (4.10 vs 0.363) and collapse its action operators — the cost of learning on raw state rather than on residual.
