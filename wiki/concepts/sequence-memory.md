...
|---|---|
| Temporally asymmetric STDP | SeqNet weight $J_{ij} = \xi_i^{\mu+1}\xi_j^\mu$ — pre→post LTP writes the forward edge $\xi^\mu \to \xi^{\mu+1}$ |
...
| Cortico-BG-thalamo-cortical (CBGT) loop | DenseNet bipartite circuit: visible (motor cortex) + hidden (thalamic motor motifs) + BG context gate |
...
The BG's role in this circuit is **not** action selection (Go/NoGo) but *context-dependent sequence gating*: inhibiting specific thalamic hidden neurons ($h_\mu$) reduces the probability of transitioning to pattern $\xi^{\mu+1}$, while activating them increases it. This is a distinct mechanistic mode for BG — selecting which sequence program drives the motor cortex, rather than selecting among competing motor actions.

---

## DNC Temporal Link Matrix: One-Shot Sequence Encoding

DenseNet (above) encodes sequences in asymmetric synaptic weights via many-shot STDP — the same directed edge $J_{ij} = \xi_i^{\mu+1}\xi_j^\mu$ accumulates over multiple exposures. The DNC (Graves et al. 2016 [[wiki/papers/dnc-graves-2016.md]]) provides a complementary mechanism: **one-shot sequence encoding in a dynamic data structure** rather than fixed weights.
...
| Property | DenseNet (weight-encoded) | DNC temporal links (structure-encoded) |
|---|---|---|
...
- **CBGT loop as program executor:** thalamic hidden units are the "program slots"; BG selects the active program context; cortex executes each step — this is the circuit-level analog of the Block 3C hierarchical rule stack.
- **MixedNet temporal flexibility:** variable $\tau$ per state allows the executor to pause at complex reasoning steps for longer (deliberate pause), consistent with the BG Hold function via hyperdirect pathway.
...
- **[[wiki/entities/basal-ganglia.md]]** — the DenseNet bipartite implementation maps onto the CBGT loop; BG's role is reframed from competing-action selection (Go/NoGo) to context-dependent gating of which sequence-memory program drives thalamic hidden units.
