...
- **[[wiki/concepts/temporal-context.md]]** — TCM's `M^{TF}` (context→item) and `M^{FT}` (item→context) are Hebbian outer-product heteroassociators; context-cued recall is content-addressable retrieval with the drifting temporal-context vector as the query, and its two-component retrieval yields the forward/backward asymmetry a symmetric Hopfield store lacks.
